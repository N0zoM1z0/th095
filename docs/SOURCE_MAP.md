...
| TH095 EnemyInf | `PhotoEnemyManager.hpp` is the single 0x26AE30 normal manager owner; task, ECL, Background, photo, operand, runtime-scan, and enemy-shot sources consume it | Exact/DIFF guarded task/runtime views retain target-facing class names and lexical shapes; `EnemyManagerUpdate.cpp` retains the full exact compact-element layout | The canonical manager owns 128 fixed-size 0x4CC0 slots without claiming the compact element is finished. `enemyAnm @ +0x4DF8` is proved; `+0x4DFC` remains opaque. The 0x9DCF10/481-slot declaration in `EnemyManager.hpp` is TH08-shaped Enemy/ECL compatibility, not this target owner. |
...
| ECL | `ecl/EclRun.cpp` with `EclRunLow.inl` / `EclRunHigh.inl`; normal `EclExtended.cpp` uses canonical ANM, Background, BulletInf, Effect, and EnemyInf manager owners | exact fragments, `EclExtended*Emission.inl`, and local target-facing views | Historical receiver names are compiler-emission adapters only. Included RunEcl handler bodies share outer lexical state; compact enemy-element, player, and camera projections remain open. |
| Legacy Enemy/ECL compatibility | `EnemyManager.hpp` plus inherited ECL consumers | selected exact inlines/local views | Its element declarations remain useful compatibility surface, but its 481-slot manager is not the TH095 EnemyInf allocation. Compact TH095 element projections still need a separate owner audit. |
