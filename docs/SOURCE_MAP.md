...
| ECL | `ecl/EclRun.cpp` with `EclRunLow.inl` / `EclRunHigh.inl`; normal `EclExtended.cpp` uses canonical `AnmVmId`, `AnmLoaded`, `Float3`, `PhotoEffectManagerView`, and `PhotoBulletManagerView` identities | exact fragments, `EclExtended*Emission.inl`, and local target-facing views | Historical ANM/math/PhotoEffect/BulletInf receiver names are compiler-emission adapters only. Included RunEcl handler bodies share outer lexical state; EnemyInf, player, and camera projections remain open. |
