...
`EBP-0x54..-0x7C` interval.  Source/target homes agree through the temporary used
by `bullet->activeTimer = 0`; the next source temporary, generated by the first
fast-spawn `bullet->position -= bullet->velocity * 4.0f`, is displaced by exactly
`0x2C`, as are all later deep homes.  A real force-inline helper containing the
three repeated spawn-offset operations leaves the frame and extent unchanged.
