...
- The PBG archive/file lane now has twenty-two canonical exact authored functions for 2,808 bytes, plus exact `CalculateByteChecksum @ 0x0041BA30` (71 bytes). New exact leaves are `PbgArchiveEntry::PbgArchiveEntry @ 0x00452E50` (23 bytes) and `CPbgFile::ReadWholeFile @ 0x00455BF0` (199 bytes). ReadWholeFile keeps TH08 ownership ancestry but uses TH095 CRT `malloc/free`, drops the redundant post-malloc null guard/null-store on read failure, and requires the fully live `{oldLocation, dataLen, data}` aggregate. `ReadDecompressEntry`, `ParseHeader`, the 16-byte THA1 layout, inline empty IPbgFile lifetime, `CopyFileName`/malloc, Load reopen, and force-inlined Release ownership rules still apply. `AllocEntries @ 0x00455580` is improved from 462/524 to 511/524 by restoring TH08 new[] failure-delete ownership and a live `{buffer,i,entryData}` aggregate; only the deep filename-size allocation phase remains. Skip `CPbgFile::Open` and do not pad AllocEntries.
