...
- The PBG archive/file lane now has eighteen canonical exact authored functions for 1,583 bytes. New exact ownership functions are the 53-byte archive constructor, 259-byte `Load`, 190-byte `Release`, and 41-byte entry destructor. TH095 `Load` reopens the archive through mode zero after parsing; `Release` requires one source-local force-inlined free helper so the real filename argument home appears after the compiler's array/virtual-delete homes. The inline empty `IPbgFile` lifecycle and TH095 `CopyFileName`/`malloc` rules still apply. Skip `CPbgFile::Open`/`ReadWholeFile` and archive `ReadDecompressEntry`/`ParseHeader`/`AllocEntries` until a new natural TH095 oracle appears.
