...
### SEM-159 — name the photography feedback sound IDs

**Scope.** Rotate away from SEM-158 SceneSelect weak-API naming into the sound/state surface named by the handoff. The first bounded audit actively tried to promote `SoundBufferIdxVolume::unconsumedMetadata` / `SoundPlayer::unconsumedMetadataBySound`, but fresh target evidence showed that metadata remains writer-only: the two play producers copy it, while `ProcessQueues` consumes queue index/count/pan, duplicate buffers, and volume without reading the copied array. This transaction therefore leaves that metadata Unknown and instead binds only the five public photography sound IDs `0x2A..0x2E`, whose TH095-local assets and independent gameplay producers agree. It does not change queue scheduling, DirectSound behavior, sound-table values, global sound suppression, camera flags, ECL opcode semantics, or any audio resource lifetime.

**Observed.** The target-backed `g_SoundBufferIdxVol[47] @ 0x004A4480` maps public IDs `0x2A..0x2E` to SFX buffer indexes `32..36`. The exact TH095 `g_SFXList[37] @ 0x004A45F8` maps those five buffers to `se_ch00.wav`, `se_ch01.wav`, `se_focus.wav`, `se_nice.wav`, and `se_focusin.wav` respectively. Their maintained volume values are `-300, -300, -300, -500, -500`; the copied third-table-word value is `100` for all five but remains semantically unconsumed by the playback consumer and is not named by this batch.

Fresh Factory-attested TH095 decompilation closes each behavioral producer. `PhotoCameraState::UpdateCharge @ 0x00433D10` starts `0x2A` after the five-frame focus acquisition and stops it when sound becomes suppressed, required focus input is released, or full charge is reached. `UpdatePhotoCamera @ 0x00430AB0` plays `0x2B` exactly when the charge presentation crosses into its full state, and plays `0x2E` only on the target-present edge guarded by the existing one-shot target-sound latch. `BeginCapture @ 0x00432730` starts `0x2C` when the camera enters charging/focus mode; `TakePhoto @ 0x00432D10`, `CancelCapture`, and the charging suppression path stop the same ID. Fresh `EclManager::RunEcl @ 0x00408E70` case `0x90` (decimal opcode 144) initializes the compact enemy's `photoPulseTimer` and `photoPulseDurationTimer`, creates ANM script `0x125`, stores `photoPulseVmId`, and then plays `0x2D`; the independently reconstructed enemy update consumes that VM/timer lifetime.

**Corroborated.** Earlier TH095-local semantic records already separate these state protocols: SEM-074/148 bind the focus-charge lifetime around `0x2A`; SEM-157 binds the full-charge presentation edge around `0x2B`; the shared photo-sound suppression record observes `0x2C` as the sustained camera focus/charging sound and `0x2E` as the target-lock feedback; SEM-098 binds opcode 144 to the transient photo-pulse VM/timer family accompanying `0x2D`. Those records are corroboration only here; the accepted names are rechecked against the current target and the exact TH095 SFX table. TH08 supplies no identifier authority.

**Inferred.** The maintenance identifiers `SOUND_FOCUS_CHARGE`, `SOUND_CHARGE_FULL`, `SOUND_CAMERA_FOCUS`, `SOUND_PHOTO_PULSE`, and `SOUND_TARGET_ACQUIRED` describe the narrow observed TH095 behavior rather than the subjective sound content. `SOUND_CAMERA_FOCUS` deliberately follows both the `se_focus.wav` asset name and its BeginCapture-to-capture/cancel lifetime; it is not claimed to be a generic UI focus sound. `SOUND_PHOTO_PULSE` means only that opcode 144 emits the cue as part of the established pulse VM protocol, not that the `se_nice.wav` file had that historical source identifier.

**Unknown / bounded.** ZUN's original enum names for all five IDs remain unknown. The Japanese-game authoring intent of `se_ch00`, `se_ch01`, `se_focus`, `se_nice`, and `se_focusin` is not inferred beyond the target-observed call contexts. This batch does not assign meaning to the SoundPlayer table's third word, `unconsumedMetadataBySound`, neighboring placeholder sound IDs, or the reason opcode 144's pulse uses `se_nice.wav`. Runtime audiovisual equivalence remains a separate unclaimed plane.

**Production / exact representation.** Normal `src/SoundPlayer.hpp` exposes the five semantic enum spellings while preserving their sequential numeric values. `TH095_MATCH_EXACT` deliberately retains the historical `SOUND_2A..SOUND_2E` lexical surface. Profile-aware `TH095_SOUND_*` aliases expand to the raw target-facing casts in exact builds and the semantic enum values in normal production; `PhotoCamera.cpp` and opcode-144 `EclRunTargetHigh.inl` use those aliases. No function signature, class layout, table layout, branch, argument value, or calling convention changes.

**Validation.** The first focused exact attempt used the new enum spellings directly in the exact-visible shared header. `photo-update-viewfinder` retained the same structural relocation surface but VC7.1 renumbered compiler-private `$L...` names; that attempt was rejected and no manifest/private-label refresh was accepted. Preserving the historical `SOUND_2A..SOUND_2E` lexical surface under `TH095_MATCH_EXACT` removed the churn. Fresh focused replay then rebuilt `PhotoCamera.cpp`, `SoundPlayer.cpp`, and `ecl/EclRun.cpp` and passed 11 + 27 + 1 = 39/39 configured units with zero compiler-private label refresh. Command-local normal-production probes with pinned VC7.1 13.10.3077 emitted 58,000-byte `PhotoCamera.obj`, 64,526-byte `SoundPlayer.obj`, and 77,946-byte `EclRun.obj` Intel 80386 COFF objects; their temporary object/PDB directory was removed by the command trap. Because `SoundPlayer.hpp` is shared broadly, a cold aggregate replay was recovered into four durable 22-source partitions after an unreceipted one-shot bridge timeout; the durable partitions passed 205 + 158 + 191 + 142 = 696/696 configured units across all 88 sources with zero private-label refresh. A fresh whole-product build compiled all 88 production translation units and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `c87ff89ef1e9dfb8d4fdb462499b3395842a3ec56382edba9e60260961f12c68`; this is compile/link closure, not whole-image byte identity. `python3 scripts/ci.py` passed all 43 tests, tracking remains 1,880 provisional / 697 source-present / 696 exact, the match graph remains 696 units, the whole-build graph remains 88 sources / 2 profiles, target verification remains SHA-256 `bb54f6fc54f0eeffaec416ca9f64aef32b5f59b7427fa5a6579f6538e0eddc07`, and `git diff --check` passed. No runtime scenario is claimed: this transaction changes identifiers/representations while preserving the same sound IDs and target control flow.

**Recovery / analysis state.** The transaction began from committed HEAD `677917b240985477dfe21e7bd26729a78dc26d95` with no staged or tracked unstaged changes and the same four pre-existing untracked paths preserved outside staging. `.analysis/` began at 3,394,984 bytes; no current-session filesystem analysis root has been created because fresh target evidence came through the registered read-only Ghidra provider. Negative routes before this batch included SoundPlayer copied-but-unconsumed metadata, compact ECL bits already bounded by SEM-104, and the declaration-only `TryDecryptFromTable` weak API. None of those negative results changes the active-incomplete phase.

**Next evidence route.** After checkpoint, rotate away from camera/audio feedback. Prefer an independent persistent/ABI or historical-runtime owner, a non-ANM resource lifetime, replay/input storage only when a new field-level consumer appears, or a distinct interpreter/state protocol with TH095-local producer plus independent consumer. Keep SoundPlayer/PBG copied metadata, FrontEnd controller bit 4, shared photo/global bit 8, ReplayScanWorker `unknown010`, replay/input reserved bytes, score/replay reserved words, photo-score bits 5/17/18/19, ANM bit 14, TextRenderer RNG prefix, compact tail storage, and other single-ended fields Unknown absent new evidence. The semantic phase remains active-incomplete.

### SEM-160 — canonicalize the persistent score-record header

**Scope.** Rotate away from SEM-159 camera/audio feedback into the persistent score ABI. SEM-094 already proved that both recognized decompressed record families share a 0x0C `{magic, version, size, checksum}` prefix, and SEM-152 proved that the 0x458-byte ST profile is simultaneously the runtime owner for replay name and scene-selection cursors. The remaining representation gap is that `ScoreLoad.cpp` still owns a TU-private `ScoreRecordHeaderView` while `ResultSaveDataView` calls the same profile prefix `unknownRuntime008[0x0C]` and `ScoreLifecycle.cpp` spells the four fields independently. This transaction canonicalizes that already-proven prefix only. It does not reinterpret the outer score-file header, profile payload after +0x0C, SC record body, checksum algorithm, compression/encryption, or any runtime selection field.

**Observed.** Fresh Factory-attested TH095 decompilation of `ScoreProfileView::Initialize @ 0x00435500` writes `0x5453` (`ST`) at profile `+0x00`, version `0` at `+0x02`, size `0x458` at `+0x04`, then begins the replay-name bytes at `+0x0C`; the enclosing save owner is zeroed before this call, so checksum `+0x08` begins zero until serialization refreshes it. Fresh `ResultSaveDataView::ParseScoreFile @ 0x004356D0` treats decompressed records through exactly the same field widths and offsets: SC is magic `0x4353`, version 1; ST is magic `0x5453`, version 0; checksum is dword `+0x08`; stream advancement subtracts and advances by dword `+0x04`. Valid ST data is copied as one 0x458-byte record into save-owner `+0x08`.

Fresh `WriteBestShotData @ 0x00435910` closes the producer side. It clears profile checksum at save-owner/profile `+0x08`, calculates the aligned checksum over the full 0x458-byte profile, writes the result back to the same `+0x08` checksum slot, then copies/serializes the complete profile after any SC records. This independently ties the runtime owner at save-owner `+0x08` to the same serialized prefix consumed by the parser rather than to an unrelated anonymous 12-byte runtime gap.

**Corroborated.** SCORE-007/SEM-094 already established the shared SC/ST prefix from parser and producer evidence; SCORE-008/SEM-152 independently established that the ST profile copied into `ResultSaveDataView::profileData @ +0x08` is the runtime owner containing `replayName`, `lastSelectedGroup`, `lastSelectedScene`, and `nextSceneByGroup`. The current batch joins those two already-proven views. TH08 is not needed for the field identity.

**Inferred.** `ScoreRecordHeaderView` is the maintainable production representation of the shared 0x0C record ABI. Promoting it to `ScoreData.hpp` does not claim that the original TH095 source used a shared C++ struct or that SC/ST records inherited from a common type; it states only that the target uses the same byte protocol and that reconstruction should not describe the same owner bytes as anonymous in one consumer and typed in another.

**Unknown / bounded.** This batch does not assign a new meaning to outer `ScoreFileHeader +0x0C = 0x102`; that remains a separate single-ended field because the score-file parser does not validate it. It does not reinterpret `unknownRuntime01d`, the random-fill/profile overlap beyond the fields already proven by SEM-152, malformed-record bounds behavior, or the semantics of unused profile tail bytes.

**Production / exact representation.** Normal `src/ScoreData.hpp` now owns `ScoreRecordHeaderView {u16 magic; u16 version; u32 size; i32 checksum;}` and exposes `ResultSaveDataView::profileRecordHeader @ +0x08` in the runtime overlay. `src/ScoreLoad.cpp` removes its duplicate TU-private type and continues to use the canonical view. Normal `src/ScoreLifecycle.cpp` embeds the canonical header at profile `+0x00` and writes ST/version/size through it. `TH095_MATCH_EXACT` preserves `unknownRuntime008[0x0C]` in the shared owner and continues to compile the frozen ScoreLifecycle exact source; no exact-facing field spelling, serialized byte, function signature, or layout changes.

**Validation.** Focused `replay-exact-units.py` rebuilt `ScoreData.cpp`, `ScoreLifecycle.cpp`, and `ScoreLoad.cpp` and passed 1 + 5 + 1 = 7/7 configured units with zero compiler-private label refresh. Command-local normal-production probes using pinned VC7.1 13.10.3077 emitted 19,561-byte `ScoreData.obj`, 14,015-byte `ScoreLifecycle.obj`, and 11,535-byte `ScoreLoad.obj` Intel 80386 COFF objects; the temporary object/PDB directory was removed by the command trap. Because `ScoreData.hpp` is shared across ResultScreen, SceneSelect, PhotoStage/PhotoGameTask, and the score TUs, the cold aggregate gate replayed all 88 manifest sources in four durable 22-source partitions and passed 205 + 158 + 191 + 142 = 696/696 configured units with zero compiler-private label refresh. A fresh whole-product build compiled all 88 production translation units and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `b2ec4dacc24a11499228c4fac1559e7ebb321e90f51b0681d0205036b67f0fb0`; this is compile/link closure, not whole-image byte identity. `python3 scripts/ci.py` passed all 43 tests, tracking remains 1,880 provisional / 697 source-present / 696 exact, the match graph remains 696 units, the whole-build graph remains 88 sources / 2 profiles, target verification remains SHA-256 `bb54f6fc54f0eeffaec416ca9f64aef32b5f59b7427fa5a6579f6538e0eddc07`, and `git diff --check` passed. No runtime scenario is added because this transaction changes only a typed persistent representation while preserving bytes and behavior.

**Recovery / analysis state.** This transaction begins from committed HEAD `49168dfeb7d4288fa6160a1e32079605038710f9` with zero staged/tracked unstaged changes and the same four pre-existing untracked paths preserved outside staging. `.analysis/` remains at the campaign-start 3,394,984 bytes with only the three legacy/currently-unowned roots untouched; fresh evidence came through the registered read-only Ghidra provider, so no current-session filesystem analysis artifact was created. Bounded negative rotations before this batch retained ReplayScanWorker `unknown010`, the TextRenderer RNG prefix, score-header `+0x0C`, photo-score bits 17/18/19, and unrelated Bullet state domains as Unknown or already-committed semantics.

**Next evidence route.** After checkpoint, rotate away from score-profile representation. Prefer an independent historical-runtime or non-ANM resource-lifetime owner, a replay/input field only when a new field-level reader appears, or a distinct interpreter/state protocol with a TH095-local producer plus independent consumer. Keep outer score-header `+0x0C`, SoundPlayer/PBG copied metadata, FrontEnd controller bit 4, shared photo/global bit 8, ReplayScanWorker `unknown010`, replay/input reserved bytes, photo-score bits 5/17/18/19, ANM bit 14, TextRenderer RNG prefix, compact tail storage, and other single-ended fields Unknown absent new evidence. The semantic phase remains active-incomplete.


### SEM-161 — bind the FileSystem source selector

**Scope.** Rotate away from SEM-160 score-profile representation into an independent historical resource-loading boundary. `FileSystem::OpenFile` already has a canonical exact 576-byte implementation and many independent TH095 callers, but its third normal-production parameter was named `isExternalResource`, while durable FILE-002 described an archive miss as falling back to loose-file I/O. Fresh target control flow falsifies that fallback claim. This transaction corrects the durable behavior statement and narrows only the normal API parameter name to `loadFromDisk`. It does not change the parameter type/value, archive format, path stripping, allocator, critical-section protocol, file-size output, caller constants, or any ownership/free rule.

**Observed.** Fresh target disassembly of `FileSystem::OpenFile @ 0x0041A960` branches on the third argument at `0x0041A996..0x0041A99A`. A nonzero value jumps directly to the Win32 loose-file block at `0x0041AA8C`, which calls `CreateFileA`, `GetFileSize`, the game allocator, `ReadFile`, and `CloseHandle`. A zero value instead strips path components and queries `g_PbgArchive`; after the decompressed-size lookup, a zero size at `0x0041AA0F..0x0041AA15` jumps to the common error path `0x0041AB77`, not to the disk block. A nonzero archive size is allocated, decompressed, and returned through the archive lane. Both lanes are serialized by critical section 2 and balance the shared active-count byte.

Fresh TH095 callers establish the source-selector polarity independently. `Supervisor::LoadConfig @ 0x00424D30` passes `1` when opening the external configuration file. `ResultSaveDataLifecycleView::ResultSaveDataLifecycleView @ 0x004354B0` passes `1` for `scoreth095.dat`. `ReplayManager::LoadReplay @ 0x00435130` passes `0` in its archive-backed branch, while its loose replay branch uses the separate shared-handle `ReplayFile` lane. Reconstructed callers corroborate the same partition: external ANM texture payloads and best-shot files use `TRUE`; packaged ECL, background, help, MIDI, BGM metadata, and scene assets use `FALSE`.

**Corroborated.** The exact `file-system-open-file` unit already binds `OpenFile` to `0x0041A960`, and caller relocation ledgers bind Main, ReplayManager, ScoreLifecycle, ANM, Background, FrontEnd, MIDI, SoundPlayer, SceneSelect, and other resource users to the same target function. FILE-003/004/005/006 separately establish the neighboring loose-file probe and serialized write/shared-handle lanes. TH08 is not needed for this interpretation.

**Inferred.** `loadFromDisk` is the narrowest maintainable Boolean name supported by TH095-local behavior. It intentionally describes source selection rather than resource provenance: `TRUE` means bypass PBG lookup and use the loose Win32 file path; `FALSE` means use the archive lane only. The name does not imply that all disk-loaded files are user-authored, optional, mutable, or outside the installation directory.

**Unknown / bounded.** ZUN's original parameter identifier remains unknown. This evidence does not establish why the interface is a Boolean rather than an enum, whether zero-length PBG entries were intentionally forbidden, or whether any historical engine version implemented archive-to-disk fallback. `TryDecryptFromTable` and PBG entry metadata remain separate Unknown surfaces.

**Production / exact representation.** Normal declarations in `Global.hpp` and `Main.hpp`, plus the natural implementation in `FileSystem.cpp`, name the third argument `loadFromDisk`; the exact-only class declaration in `Main.hpp` retains `isExternalResource`. Caller literals and generated ABI are unchanged. FILE-002 is corrected to state the mutually exclusive archive-only/disk-only lanes, and FILE-007 records the semantic parameter contract.

**Validation.** Focused replay rebuilt all four configured `FileSystem.cpp` units and passed 4/4 exact, then rebuilt the representative Main, ReplayManager, and ScoreLifecycle caller surfaces and passed another 48 + 12 + 5 = 65/65 units; all 69 focused units passed with zero compiler-private label refresh. A command-local normal-production probe using pinned VC7.1 13.10.3077 emitted a 20,920-byte Intel 80386 COFF `FileSystem.obj`; its temporary object/PDB directory was removed by the command trap. Because `Global.hpp` and `Main.hpp` are shared headers, the cold aggregate replay covered all 88 manifest sources in four durable 22-source partitions and passed 205 + 158 + 191 + 142 = 696/696 configured units with zero private-label refresh. A fresh whole-product build compiled all 88 production translation units and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `00ed98aaf370315d00328ac2d7bb3ef7d541babe13b8b81625d341620da61b6a`; this is compile/link closure, not whole-image byte identity. `python3 scripts/ci.py` passed all 43 tests, tracking remains 1,880 provisional / 697 source-present / 696 exact, the match graph remains 696 units, the whole-build graph remains 88 sources / 2 profiles, target verification remains SHA-256 `bb54f6fc54f0eeffaec416ca9f64aef32b5f59b7427fa5a6579f6538e0eddc07`, and `git diff --check` passed. Wine emitted only GUI-driver/systray diagnostics during tool execution; no runtime scenario is claimed for this identifier-only representation.

**Recovery / analysis state.** The transaction begins from committed HEAD `087fbb72a6232e620adaea161400779cb298aeb7` with zero staged/tracked unstaged changes and the same four pre-existing untracked paths preserved outside staging. `.analysis/` began at 3,394,984 bytes; no current-session filesystem analysis root has been created because fresh evidence came through the registered read-only Ghidra provider. Negative rotations before this batch retained Supervisor anonymous gaps, the already-committed FrontEnd queue reset offsets, compact-enemy shot cadence/spawn descriptor offsets, and previously bounded writer-only/single-ended fields without promotion. None changes the active-incomplete phase.

**Next evidence route.** After checkpoint, rotate away from FileSystem source selection. Prefer a distinct non-file historical-runtime owner, a non-ANM resource lifetime with a real create/use/release relation, or an interpreter/state protocol with a new TH095-local producer plus independent consumer. Keep outer score-header `+0x0C`, SoundPlayer/PBG copied metadata, FrontEnd controller bit 4, shared photo/global bit 8, ReplayScanWorker `unknown010`, replay/input reserved bytes, photo-score bits 5/17/18/19, ANM bit 14, TextRenderer RNG prefix, Supervisor gap storage, compact tail storage, and other single-ended fields Unknown absent new evidence. The semantic phase remains active-incomplete.


### SEM-162 — align the slot-nine photo ANM release name

**Scope.** Rotate away from SEM-161 file-source selection into a bounded non-file resource lifetime. Front-end resource loading already names `LoadPhotoAnm`, but the matching slot-nine teardown wrapper was still named `ReleaseResultAnm` in normal production. Fresh TH095-local caller and callee evidence proves that the two functions are the load/release endpoints of the same `photo.anm` slot-9 resource. This transaction changes only the normal-production releaser identifier and its normal Main call. It does not change ANM slot ownership, preload/release behavior, front-end load order, shutdown order, VM retirement, or any exact-facing symbol/ledger identity.

**Observed.** Fresh target `LoadPhotoAnm @ 0x0042AA90` calls `AnmManager::PreloadAnm` with index 9 and the literal `"photo.anm"`, logs the existing Japanese load-error diagnostic on null, and otherwise returns zero. Factory target callers show exactly one caller, `FrontEndLifecycleView::LoadResources @ 0x00445A50`. Fresh decompilation of that caller loads resources in the sequence photo-bullet ANM, `0x0042AA90`, ResultScreen ANM, PhotoFront ANM, then PhotoPlayer ANM before clearing the asynchronous load-active flag.

Fresh target `0x0042AAD0` contains only `AnmManager::ReleaseAnm(9)` followed by return zero. Its sole target caller is `Supervisor::DeletedCallback @ 0x004244D0`, where it immediately follows the photo-bullet ANM release and precedes ResultScreen/replay, PhotoFront, front-end, and PhotoPlayer teardown. The common slot index 9 closes the resource identity with the `photo.anm` loader independently of the inherited function spelling.

**Corroborated.** Canonical exact units already bind `LoadPhotoAnm @ 0x0042AA90` to the `"photo.anm"` literal and slot-nine `PreloadAnm`, and bind exact-facing `ReleaseResultAnm @ 0x0042AAD0` to slot-nine `ReleaseAnm`. The exact `front-end-load-resources` caller relocation points to `LoadPhotoAnm`; the exact `main-deleted-callback` relocation points to `ReleaseResultAnm`. These ledgers establish target identity but do not require the normal production identifier to repeat the inherited semantic mismatch.

**Inferred.** `ReleasePhotoAnm` is the maintainable normal name because the function's complete TH095-local behavior is release of the same slot whose unique preload helper is `LoadPhotoAnm`. This does not claim that ZUN's original source used the name `ReleasePhotoAnm`; the exact-facing historical reconstruction spelling remains `ReleaseResultAnm` because that token participates in the canonical exact symbol surface.

**Unknown / bounded.** The historical reason for the `ReleaseResultAnm` spelling remains unknown. It may reflect an ancestor name, a broader historical use, or a reconstruction-era label; no claim is made among those possibilities. This batch does not reinterpret ResultScreen's independent ANM resource, slot-nine preload reference counting, or whether another engine version shared the slot differently.

**Production / exact representation.** Normal `src/PhotoOverlay.cpp` now exports `ReleasePhotoAnm`, and normal `src/Main.cpp` declares/calls that name during Supervisor teardown. `TH095_MATCH_EXACT` continues to include the frozen `PhotoOverlayExact.inl` and `MainExact.inl`, preserving exact-facing `ReleaseResultAnm`, its decorated symbol, the match-unit manifest, and all target relocations. No shared header or object layout changes.

**Validation.** The first focused-validation request failed at the Factory transport boundary with `network_error` before returning a command id, so it receives no validation credit. Recovery confirmed the four tracked transaction edits were intact, the same four pre-existing untracked files were unchanged, `.analysis/` remained 3,394,984 bytes, and no replay/compiler/Wine producer survived. The replay was then issued durably: `Main.cpp` passed 48/48 and `PhotoOverlay.cpp` passed 11/11 configured units, for 59/59 exact with zero compiler-private label refresh. Command-local normal-production probes using pinned VC7.1 13.10.3077 emitted a 114,902-byte `Main.obj` and 23,596-byte `PhotoOverlay.obj`, both Intel 80386 COFF; symbol inspection showed `PhotoOverlay.obj` defining and `Main.obj` referencing `?ReleasePhotoAnm@th095@@YIHXZ`. Their temporary object/PDB directory was removed by the command trap. Because this is TU-local naming with no shared header/layout change, aggregate exact and whole-product closure are deferred only until the immediately following committed campaign milestone/final handoff, where they will be replayed against the post-SEM-162 source tree. `git diff --check` passed.

**Recovery / analysis state.** This transaction begins from committed HEAD `4b1b2c8a91699eff50b4fb6f2679e377eeeb9fe5` with zero staged/tracked unstaged changes and the same four pre-existing untracked paths preserved outside staging. `.analysis/` remains at 3,394,984 bytes; no current-session filesystem analysis root has been created. Fresh evidence came from the registered read-only target-attested Ghidra provider. The semantic phase remains active-incomplete.

**Next evidence route.** After checkpoint, rotate away from slot-nine ANM naming. Prefer a distinct historical-runtime state owner, a different resource lifetime with producer plus independent cleanup/consumer, or a primary/sibling interpreter protocol with a newly exposed TH095-local edge. Keep the previously bounded single-ended fields and metadata Unknown absent new evidence. The semantic phase remains active-incomplete.


### SEM-163 — canonicalize ANM preload call sites

**Scope.** Rotate from SEM-162 slot-nine naming into the shared ANM acquisition protocol and actively falsify the assumption that normal `AnmManager::LoadAnm` call sites represent the target's synchronous loader. Current exact ledgers contain a systematic proxy-name mismatch: thirteen relocations are decorated as `LoadAnm` but target `PreloadAnm @ 0x004432E0`. In the reconstructed Windows product those same source expressions resolve to the real canonical `LoadAnm @ 0x00443010`, changing slot lifetime behavior. This transaction corrects those thirteen normal-production calls while preserving their exact/DIFF decorations. The sole target-backed synchronous `LoadAnm` use in `Supervisor::AddedCallback` remains unchanged.

**Observed.** Fresh target-attested caller enumeration finds exactly one caller of `LoadAnm @ 0x00443010`: `Supervisor::AddedCallback @ 0x00423E70`, which synchronously acquires slot 2 `nowloading.anm`. Fresh target enumeration finds sixteen callers of `PreloadAnm @ 0x004432E0`: Ascii `AddedCallback`, Background `LoadStageDataInner`, PhotoBullet manager initialization and front-end loader, PhotoEnemy `LoadResources`, PhotoFront initialization and loader, PhotoEffect initialization, Supervisor `StartupThread`, ResultScreen initialization and front-end loader, PhotoOverlay initialization and loader, PhotoPlayer initialization and loader, and FrontEnd initialization.

Fresh target decompilation distinguishes the two callees. `LoadAnm @ 0x00443010` always calls `ReadAnmEntries @ 0x00443070`, sets `postloadEntryNumber` to 1, and synchronously calls `PostloadAnmEntry` until the cursor clears. `ReadAnmEntries` detects an occupied slot, sets its `releasePending` latch, and waits until `ServicePreloadedAnims @ 0x004438E0` calls `ReleaseAnm`. `PreloadAnm @ 0x004432E0`, by contrast, returns the existing slot immediately when `rawData` is already present; only an empty slot enters `ReadAnmEntries`, after which it waits for serviced postload completion or the replay-worker stop signal. `ReleaseAnm @ 0x00443980` retires owned VMs, releases texture entries, frees all slot allocations, and zeroes the complete slot.

**Corroborated.** The canonical ANM-025/ANM-043 records already establish the synchronous-load versus reuse-preload algorithms and the serviced postload cursor. Exact relocation inventory identifies thirteen affected calls whose historical symbol spelling is `LoadAnm` but whose relocation target is `0x004432E0`: two in AsciiManager, one Background, two PhotoBullet, one PhotoEnemy, one PhotoEffect, two PhotoFront, two PhotoPlayer, and two ResultScreen. Four additional target callers already use a `PreloadAnm` spelling. The one true `LoadAnm` relocation targets `0x00443010` from Main. TH08 is not needed for this interpretation.

**Inferred.** These thirteen sites participate in a reusable preload-slot lifetime: front-end warm loading and later subsystem initialization are allowed to observe the same loaded slot without forcing a release/re-read cycle. Resolving their normal source to canonical `LoadAnm` incorrectly turns reuse into replacement, changes synchronization through `releasePending`, and can make the reconstructed runtime reload resource bytes that the target reuses. The compatibility helper therefore describes the semantic operation (`PRELOAD`) while isolating the historical exact decoration.

**Unknown / bounded.** Original ZUN source identifiers for the thirteen proxy-decorated calls remain unknown, as does why their reconstructed exact declarations historically used `LoadAnm`. This batch does not claim reference counting, does not alter slot indices or filenames, and does not change the synchronization fix in ANM-013. It does not reinterpret raw ANM formats, `releasePending` beyond its established replacement role, preload-slot path bytes, or unrelated ANM flag bit 14.

**Production / exact representation.** `AnmManager.hpp` now exposes `TH095_ANM_PRELOAD_COMPAT(manager, index, path)`: exact/DIFF expands to the historical `LoadAnm` call so existing decorated relocations remain intact, while reconstructed production expands to canonical `PreloadAnm`. The thirteen affected expressions in AsciiManager, Background, BulletManager, EnemyManagerUpdate, PhotoEffect, PhotoFront, PhotoGame, and ResultScreen use that helper. `Supervisor::AddedCallback` continues to call real `LoadAnm(2, "nowloading.anm")` directly. No field layout, slot count, filename, calling convention, or target-exact relocation address changes.

**Validation.** Focused replay rebuilt all configured units in the eight affected translation units and passed 17 + 20 + 35 + 22 + 34 + 11 + 22 + 24 = 185/185 exact with zero compiler-private label refresh. Command-local normal-production probes using pinned VC7.1 13.10.3077 emitted eight Intel 80386 COFF objects: AsciiManager 38,374 bytes, Background 51,923, BulletManager 66,492, EnemyManagerUpdate 50,720, PhotoEffect 67,991, PhotoFront 32,421, PhotoGame 50,625, and ResultScreen 74,990. Symbol inspection required every object to reference `?PreloadAnm@AnmManager@th095@@QAEPAUAnmLoaded@2@HPBD@Z` and rejected any reference to canonical `?LoadAnm@AnmManager@th095@@QAEPAUAnmLoaded@2@HPBD@Z`; all eight passed. A source-wide normal audit then found exactly one remaining direct `g_AnmManager->LoadAnm` call, the target-backed slot-2 `nowloading.anm` acquisition in `Supervisor::AddedCallback`. The command-local probe directory was removed by its trap.

Because `AnmManager.hpp` is shared, cold aggregate replay covered all 88 manifest sources in four durable 22-source partitions and passed 205 + 158 + 191 + 142 = 696/696 configured units with zero private-label refresh. A fresh whole-product build compiled all 88 production translation units and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `84635ba39322f4148e0345dd4afcf73ceef452aa813949d2abf08ceb661e888d`; this is compile/link closure, not whole-image byte identity. The production map contains distinct canonical `LoadAnm` and `PreloadAnm` definitions, so the compatibility macro does not collapse their implementations. `python3 scripts/ci.py` passed all 43 tests, tracking remains 1,880 provisional / 697 source-present / 696 exact, the match graph remains 696 units, the whole-build graph remains 88 sources / 2 profiles, target verification remains SHA-256 `bb54f6fc54f0eeffaec416ca9f64aef32b5f59b7427fa5a6579f6538e0eddc07`, and `git diff --check` passed. Wine emitted only GUI-driver/systray diagnostics during tool execution; no runtime scenario is claimed. Runtime storage/scenario validation remains a separate plane.

**Recovery / analysis state.** This transaction begins from committed HEAD `b249384e6298d307e23acecde3bb911c7d612450` with zero staged/tracked unstaged changes and the same four pre-existing untracked paths preserved outside staging. `.analysis/` is 3,394,984 bytes and no current-session filesystem analysis root exists; fresh semantic evidence came through the registered read-only Ghidra provider plus repository ledgers. Two earlier read-only Factory requests lost transport before returning command ids; each received no evidence credit, and live recovery confirmed the committed tree, excluded untracked paths, `.analysis`, and producer state were unchanged. The first edit command then failed before any write because its `AnmManager.hpp` insertion anchor did not match; Factory after-state and the recovery gate both confirmed zero tracked changes before this corrected edit was applied once. The semantic phase remains active-incomplete.

**Next evidence route.** After checkpoint, rotate away from the ANM load/preload family. Prefer an independent historical-runtime state owner, a non-ANM resource lifetime, or an interpreter/persistent protocol with a new TH095-local producer plus independent consumer. Keep ANM bit 14, preload-slot path bytes, shared photo bit 8, FrontEnd controller bit 4, ReplayScanWorker `unknown010`, SoundPlayer/PBG copied metadata, TextRenderer RNG prefix, Bullet anonymous tails, score-header `+0x0C`, and other single-ended storage Unknown absent new evidence. The semantic phase remains active-incomplete.


### SEM-164 — restore the OnDraw2 background viewport selector

**Scope.** Rotate away from SEM-163 ANM acquisition into an independent historical-runtime viewport protocol. A duplicate-relocation audit found one exact-facing `Supervisor::ConfigureGameplayViewport` relocation whose target is not canonical gameplay selector `0x00404B10`: `Supervisor::OnDraw2 @ 0x004235D0` targets `0x00401B70`. Normal `Main.cpp` nevertheless called real `ConfigureGameplayViewport(1)`, so the reconstructed product selected the correct Supervisor slot but applied the wrong transform protocol. This transaction changes only the normal OnDraw2 selector and corrects the durable MAIN-010 statement.

**Observed.** Fresh target `Supervisor::OnDraw2 @ 0x004235D0` begins with a call to `0x00401B70` using index 1. Target `ConfigureBackgroundViewport @ 0x00401B70` publishes `this + 0x1E4 + index * 0xF0` to active pointer `+0x3C4`, calls `ApplyBackgroundViewport @ 0x00425AA0`, sends the selected D3D viewport through `SetViewport`, and publishes the index at `+0x3C8`. Target `ConfigureGameplayViewport @ 0x00404B10` has the same owner/slot-selection skeleton but instead calls `ApplyGameplayViewport @ 0x00425910`. MAIN-022 already establishes that the background apply path builds the 3D view from camera position/look-at/up and updates the ANM shake pair, while the gameplay apply path derives the centered 2D camera and gameplay projection.

Exact relocation inventory independently partitions the caller set. `0x00401B70` is used by Main OnDraw2, Ascii background-text drawing, and Background object rendering. `0x00404B10` is used by GameWindow rendering, Background high-priority compositing, ANM layers 7/8, and the PhotoOverlay draw callback. The Main OnDraw2 relocation alone carries the misleading decorated spelling `ConfigureGameplayViewport` while targeting `0x00401B70`.

**Corroborated.** `Main.hpp` and `SupervisorRuntime.hpp` expose both canonical production methods, and `Background.cpp` already implements production `Supervisor::ConfigureBackgroundViewport` separately from `SupervisorViewport.cpp`'s gameplay selector. ABI-073/075 and MAIN-022 establish the shared active viewport owner and the distinct background/gameplay projection behavior. No TH08 evidence is needed.

**Inferred.** OnDraw2 requires the background transform protocol even though it selects full viewport configuration 1. Using the gameplay selector is not a harmless alias: it changes the view/projection matrices and omits the background-path basis/shake publication. The semantic correction is therefore the callee identity, not the index or viewport storage.

**Unknown / bounded.** The historical reason for the exact-facing `ConfigureGameplayViewport` token remains unknown. This batch does not rename either canonical target function, does not change the two `0xF0` configuration layouts, and does not reinterpret the other selector callers. In particular GameWindow::Render, Background high-priority drawing, ANM layer 7/8, and PhotoOverlay remain genuine gameplay-selector consumers.

**Production / exact representation.** Normal `Supervisor::OnDraw2` now routes through `ConfigureBackgroundViewport(1)`. A local `TH095_ON_DRAW2_CONFIGURE_VIEWPORT` compatibility macro preserves the historical `ConfigureGameplayViewport` call only for DIFFBUILD; `TH095_MATCH_EXACT` remains frozen in `MainExact.inl` and retains its original token/relocation. No shared header or object layout changes.

**Validation.** Focused replay rebuilt all 48 configured `Main.cpp` units and passed 48/48 exact with zero compiler-private label refresh; the exact lane remains frozen in `MainExact.inl`. A command-local normal-production probe using pinned VC7.1 13.10.3077 emitted a 114,975-byte Intel 80386 COFF `Main.obj`. Relocation inspection found exactly one `ConfigureGameplayViewport` reference and one `ConfigureBackgroundViewport` reference, matching the target Main partition: `GameWindow::Render` remains the gameplay caller while `Supervisor::OnDraw2` is the background caller. The temporary probe directory was removed by its trap.

Because the correction changes a cross-object callee, a fresh whole-product build immediately compiled all 88 production translation units and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `53e3a6b505052754aaa2907cc8664ff544bc6e7043bd181724e18f6c41c9f3fb`. The linked map retains distinct `Supervisor::ConfigureBackgroundViewport` in `Background.obj` and `Supervisor::ConfigureGameplayViewport` in `SupervisorViewport.obj`; this is compile/link closure, not whole-image byte identity. `python3 scripts/ci.py` passed all 43 tests, target verification remained canonical, tracking remained 1,880 provisional / 697 source-present / 696 exact, and `git diff --check` passed. Wine emitted only GUI-driver/systray diagnostics; no runtime scenario is claimed. Since no shared header/layout changed, cold aggregate exact is deferred to the final committed campaign milestone.

**Recovery / analysis state.** This transaction begins from committed HEAD `0550dcd6c4453b0f9114932e05aa3d6465507590` with zero staged/tracked unstaged changes and the same four pre-existing untracked paths preserved outside staging. `.analysis/` remains 3,394,984 bytes with no current-session filesystem analysis root. During the recovery gate, `origin/main` was observed to advance externally to the same `0550dcd` commit (`refs/remotes/origin/main` reflog says `update by push`); this campaign did not run `git push` and did not alter the remote ref itself. The semantic phase remains active-incomplete.

**Next evidence route.** After checkpoint, continue the duplicate-relocation/proxy audit outside ANM and viewport protocols. Prefer `Supervisor::ThreadClose` or `GameErrorContext` only if normal production still collapses distinct target callees; otherwise rotate to a persistent/interpreter/resource domain with a new TH095-local edge. Keep the established Unknown list unchanged absent new evidence. The semantic phase remains active-incomplete.


### SEM-165 — restore Background error escalation

**Scope.** Continue the SEM-164 duplicate-relocation/proxy audit outside the ANM and viewport protocols. The exact Background ledger contains a symmetric GameError proxy mismatch: `Background::Initialize @ 0x00402250` carries a `GameErrorContext::Log` relocation decoration whose target is canonical `Fatal @ 0x0041B280`, while `Background::LoadStageDataInner @ 0x00402C80` carries a `Fatal` decoration whose target is canonical `Log @ 0x0041B0F0`. Normal production previously resolved those source spellings to the real canonical functions and therefore inverted the target's diagnostic severity. This transaction corrects only that two-call escalation protocol. It does not change error strings, stage-data control flow, ANM acquisition, `GameErrorContext` layout, critical-section behavior, or any exact-facing relocation identity.

**Observed.** Fresh Factory-attested TH095 decompilation of `Background::Initialize @ 0x00402250` shows `LoadStageData` first; when it returns nonzero, the target calls `0x0041B280` with the generic stage-data failure string and returns `-1`. Fresh decompilation of `Background::LoadStageDataInner @ 0x00402C80` shows the slot-4 stage ANM acquisition through `PreloadAnm @ 0x004432E0`; when that result is null, the target calls `0x0041B0F0` with the more specific missing/corrupt stage-ANM diagnostic and returns `-1`.

Fresh target decompilation independently distinguishes the two error callees. `GameErrorContext::Log @ 0x0041B0F0` formats into an 8 KiB temporary and appends to the shared 8 KiB context buffer under critical-section lane 3 without publishing a fatal latch. `GameErrorContext::Fatal @ 0x0041B280` formats into a 512-byte temporary, appends through the same owner, and writes byte `1` to context `+0x2004`, the established `showMessageBox` field. Thus the outer call is observably stronger than the inner call, not an interchangeable naming choice.

**Corroborated.** The exact `background-initialize` ledger records its offset-`0x29` `Log`-decorated REL32 targeting `0x0041B280`; the exact `background-load-stage-data-inner` ledger records its offset-`0x0A7` `Fatal`-decorated REL32 targeting `0x0041B0F0`. ERROR-001 already establishes the canonical Log/Fatal behavior and owner offsets. The caller chain supplies independent TH095-local protocol evidence: `LoadStageDataInner` returns `-1`, `LoadStageData` propagates that failure, and `Initialize` then performs the fatal publication. TH08 is not needed for this interpretation.

**Inferred.** The maintainable production protocol is inner diagnostic `Log` followed by outer failure escalation `Fatal`. The opposite exact relocation symbol spellings are compatibility decorations, not semantic authority for the runnable reconstructed call targets. Preserving them in exact/DIFF mode while routing normal production to the canonical target callees retains both truths without inventing duplicate error functions.

**Unknown / bounded.** The historical reason that these two exact relocation decorations are swapped remains unknown, as do ZUN's original source identifiers at the two call sites. This batch does not claim that every Background failure follows the same two-stage policy, nor does it infer when the UI consumes `showMessageBox` beyond the already-established GameError owner behavior. No deterministic runtime scenario was added for deliberately corrupting or withholding stage assets; the runtime-scenario plane for this failure path therefore remains unexercised rather than being inferred from compile/link success.

**Production / exact representation.** `src/Background.cpp` now routes the two calls through local compatibility macros. `TH095_MATCH_EXACT` and `DIFFBUILD` retain `Log` at `Initialize` and `Fatal` at `LoadStageDataInner`, preserving the configured exact relocation symbols. Normal production expands those same sites to `Fatal` and `Log`, respectively, matching the actual TH095 target callees and severity. No function signature, field layout, message byte, return value, or target-exact call destination changes.

**Validation.** Focused replay rebuilt `src/Background.cpp` and passed all 20/20 configured units exact with zero compiler-private label refresh. A command-local normal-production probe using pinned VC7.1 13.10.3077 emitted a 51,931-byte Intel 80386 COFF `Background.obj`. The cold aggregate exact gate then replayed all 88 manifest sources in four durable 22-source partitions and passed 205 + 158 + 191 + 142 = 696/696 configured units with zero private-label refresh.

A fresh whole-product build compiled all 88 production translation units with pinned VC7.1 and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `2123413f214ead98de593782decaf8c065e4ec37b20047815a85c524c1864179`; this is production compile/link closure, not a whole-image byte-identity claim. The linked map places production `Background::Initialize` at `0x0040D450`, `Background::LoadStageDataInner` at `0x0040E0E0`, `GameErrorContext::Log` at `0x00420190`, and `Fatal` at `0x00420320`; linked-image disassembly shows the outer failure call resolving to `0x00420320` and the inner ANM failure call resolving to `0x00420190`. `python3 scripts/ci.py` passed all 43 tests, `validate-tracking.py --require-target` remains 1,880 provisional / 697 source-present / 696 exact, and `git diff --check` passed. Runtime storage/scenario validation remains a separate, unclaimed plane.

**Recovery / analysis state.** This conversation began from committed HEAD `bd97b9e08be1e4d4d485857284757b483512915a` with zero staged/tracked unstaged changes and the same four documented pre-existing untracked paths preserved outside staging: user-owned `droid.resume.txt`, runtime experiments `config/runtime-scenarios.json` and `scripts/runtime-diff.py`, and unknown-origin generated-looking `EnemyManagerUpdate.i`. `.analysis/` began and remains 3,394,984 bytes; no current-session filesystem analysis root was created because fresh semantic evidence came through the registered read-only target-attested Ghidra provider. One initial combined broad exact/product/CI request lost the Factory bridge before returning a durable command id; recovery showed unchanged repository state, no surviving replay/compiler/linker/Wine/Ghidra producer, and only a rebuilt product timestamp, so that attempt received no validation credit. The durable focused, partitioned aggregate, whole-product, and CI runs above supersede it. An optional command-local assembly-listing probe was rejected before execution and likewise received no evidence credit.

**Next evidence route.** After checkpoint, continue the exact-relocation proxy audit across a different subsystem by grouping identical relocation decorations that target multiple canonical addresses. `Supervisor::ThreadClose @ 0x00425150` is not currently a positive mismatch candidate: its inspected exact relocations still target the canonical `0x00425150`, so do not manufacture work there. Prefer the next TH095-local multi-target symbol family only when normal production resolves a proxy spelling to behavior different from the recorded target; otherwise rotate to a persistent/interpreter/resource family with a producer plus independent consumer. Keep existing single-ended Unknown storage Unknown absent new evidence. The semantic phase remains active-incomplete.


### SEM-166 — restore Render replay-worker shutdown

**Scope.** Continue the post-SEM-165 relocation-proxy audit and falsify the earlier bounded assumption that `Supervisor::ThreadClose` had no live normal-production mismatch. Grouping every REL32 decoration by destination shows four `ThreadClose`-decorated Main relocations split across two target callees: the two `GameWindow::Render @ 0x00420770` exit paths target `StopReplayScan @ 0x00425640`, while the two `Supervisor::StartupThread @ 0x004242B0` teardown paths target canonical `ThreadClose @ 0x00425150`. Normal `Main.cpp` previously called `ThreadClose` at all four sites. This transaction changes only the two Render exits to the cooperative replay-worker stop protocol while preserving the exact/DIFF relocation spellings and genuine StartupThread close calls.

**Observed.** Fresh Factory-attested TH095 decompilation of `GameWindow::Render @ 0x00420770` shows the regular frame spine through ANM flush, gameplay viewport selection, calc-chain execution, and sound queue processing. When the calc chain returns `0`, target code calls `0x00425640` and returns render result 1; when it returns `-1`, it calls the same `0x00425640` and returns render result 2. Fresh xrefs confirm both Render calls among the target users of `StopReplayScan`.

Target `Supervisor::StopReplayScan @ 0x00425640` is a thin wrapper around `ReplayScanWorker::Stop @ 0x0041BB20` on the embedded worker at Supervisor `+0x648`. The established worker stop routine publishes `exitSignal = 1` and `active = 0`, waits in 200-ms intervals while reasserting that handshake, then closes the worker handle and clears its callback. By contrast, fresh target decompilation of `Supervisor::ThreadClose @ 0x00425150` enters critical-section lane 6, directly closes and clears the same handle if nonzero, clears the worker active field, and leaves the section; it does not execute the cooperative wait loop.

**Corroborated.** Exact relocation inventory partitions the ambiguous spelling independently of decompilation. `main-render` has two `?ThreadClose@Supervisor...` REL32 entries at offsets `0xBA` and `0xD4`, both targeting `0x00425640`; `main-startup-thread` has two identically decorated entries at offsets `0x196` and `0x1D2`, both targeting `0x00425150`. Target xrefs to `0x00425150` are only the two StartupThread sites, while `0x00425640` has the broader replay/front-end/scene stop caller set including the two Render exits. REPLAY-020 and ABI-085 already establish the `ReplayScanWorker::Stop` exit-handshake/wait semantics. No TH08 evidence is required.

**Inferred.** Render exit/restart is a worker lifetime boundary that must join the active replay-scan worker before control leaves the render loop. Treating its historical `ThreadClose` decoration as canonical behavior can close the OS handle while the worker is still running, bypassing the target's exit handshake and wait. StartupThread owns a different cleanup phase and intentionally retains direct `ThreadClose`; the fix is therefore caller partitioning, not renaming or replacing either canonical callee.

**Unknown / bounded.** The historical reason the two Render relocations carry the `ThreadClose` decoration remains unknown, as do ZUN's original source-level names at those two call sites. This batch does not reinterpret other `StopReplayScan` callers, the unknown role of the second ReplayScanWorker-shaped Supervisor member at `+0x7A0`, or `ReplayScanWorker::unknown010`. No deterministic runtime scenario was introduced that forces calc-chain exit while a replay scan is active, so runtime-scenario validation for the race-prevention consequence remains unexercised rather than inferred from the product build.

**Production / exact representation.** Normal `src/Main.cpp` routes the two Render exit calls through local `TH095_RENDER_STOP_REPLAY_SCAN`, which expands to `g_Supervisor.StopReplayScan()` in reconstructed production. DIFFBUILD expands the helper to the historical `g_Supervisor.ThreadClose()` token, and `TH095_MATCH_EXACT` remains frozen in `MainExact.inl`. The two later StartupThread calls remain direct `ThreadClose()` in normal production. No shared header, worker layout, callback ABI, return value, or exact relocation destination changes.

**Validation.** Focused replay rebuilt all 48 configured `Main.cpp` units and passed 48/48 exact with zero compiler-private label refresh. A fresh whole-product build then compiled all 88 production translation units with pinned VC7.1 and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `45201ab5c6f5f8191c8f9fbddd3a4f9223b1cab52464414241997b4cf8f0dfdf`; this is production compile/link closure, not whole-image byte identity. The product map places `GameWindow::Render` at `0x00422430`, canonical production `ThreadClose` at `0x00425650`, and `StopReplayScan` at `0x0043D690`. Linked-image disassembly shows Render's two early exit calls at `0x004224E9` and `0x00422503` both resolving to `0x0043D690`. The same product's `StartupThread @ 0x00424E30` retains two calls to `ThreadClose @ 0x00425650`, proving the canonical direct-close path was not globally redirected.

Because this correction changes a cross-object callee, the cold aggregate exact gate was rerun against the SEM-166 source state rather than reusing SEM-165 receipts. All 88 manifest sources passed in four durable 22-source partitions: 205 + 158 + 191 + 142 = 696/696 configured units exact with zero private-label refresh. `python3 scripts/ci.py` passed all 43 tests, `validate-tracking.py --require-target` remains 1,880 provisional / 697 source-present / 696 exact, and `git diff --check` passed. Runtime storage/scenario validation remains a separate, unclaimed plane.

**Recovery / analysis state.** This transaction starts from committed SEM-165 checkpoint `fb9d07b0dd9ad069a67ac18f367497bbb536b665`. The four pre-existing untracked paths remain excluded from staging and unchanged in ownership/classification. `.analysis/` remains 3,394,984 bytes with no current-session filesystem analysis root; fresh semantic evidence came from repository ledgers plus the registered read-only target-attested Ghidra provider. The first source-edit command failed its final anchor assertion before writing the file; Factory after-state confirmed zero tracked changes, after which the corrected bounded edit was applied once. The semantic phase remains active-incomplete.

**Next evidence route.** The relocation-collision sweep also rediscovered `PhotoResetTargetView::ResetForPhotoTransition`, but ABI-039 already split that three-destination proxy into the canonical enemy, bullet, and effect operations in normal production; do not redo it absent contradictory evidence. The ANM, viewport, GameError, and ThreadClose multi-target semantic proxies are now represented by their target callees in normal source while exact/DIFF decorations remain isolated. `__CIcos` is the remaining multi-destination repeated decoration from this mechanical sweep and should be treated first as compiler/runtime exactness provenance, not promoted into a semantic owner without TH095-local behavioral evidence. Rotate next to a persistent/interpreter/resource/state family with a producer plus independent consumer, or to another proxy only if a fresh target-local destination mismatch appears. The semantic phase remains active-incomplete.


### SEM-167 — restore the photography HUD GUI text queue

**Scope.** Resume from SEM-166 and first follow the committed interpreter-state route. The small anonymous overlays in `src/ecl/EclRun.cpp` proved to be padding around fields already established by SEM-089/090/139 and the existing score/photo-card owner records, so they did not justify new names. Persistent Score/Replay gaps likewise exposed no new reader: the outer score-header `+0x0C`, replay-header reserved bytes, `ReplayInputSource::unknown02a/unknown036`, and Result photo-slot `+0x2202/+0x2206` remain single-ended or unconsumed. A target-xref probe of anonymous Supervisor gaps also found no direct business references. Rotating to an exact-relocation proxy audit then exposed this live production mismatch: all five `PhotoGameTaskView::DrawHud @ 0x00418420` calls are historically decorated as `PhotoAsciiManagerTaskView::AddFormatText`, but every relocation targets canonical `AsciiManager::AddGuiFormatText @ 0x00401660`; normal production had resolved those source calls to the distinct regular formatter at `0x00401610`. This transaction corrects only that five-call HUD queue partition. It does not change the formatter implementations, ASCII manager layout, text strings, HUD positions/colors, other ASCII callers, or exact-facing source.

**Observed.** Fresh Factory-attested TH095 decompilation of `PhotoGameTaskView::DrawHud @ 0x00418420` shows five calls to `0x00401660`, for `"HiScore %.7d"`, `"  Score %.7d"`, `"Photo %.2d/%.2d"`, and the normal/extra scene labels. Target xrefs to `0x00401660` contain exactly those five DrawHud call sites (`0x0041854F`, `0x00418584`, `0x004185E4`, `0x00418634`, and `0x0041866E`). By contrast, xrefs to `AddFormatText @ 0x00401610` come from other Main, ReplayManager, ResultScreen, and scene/front-end text paths, not from DrawHud.

Fresh target decompilation independently distinguishes the two formatter protocols. `AddFormatText @ 0x00401610` formats into its 512-byte temporary and calls `AddString @ 0x00401450`. That regular writer increments the count at AsciiManager `+0x8064` and stores the record in the 256-entry array rooted at `+0x864`. `AddGuiFormatText @ 0x00401660` instead calls `AddGuiString @ 0x00401530`, whose independent count is at `+0x8068` and whose 64-entry array begins at `+0x6864`; the GUI formatter additionally returns the formatted byte length. Thus the target call distinction selects separate queue ownership, not merely an alternate name for identical formatting.

**Corroborated.** The canonical exact ledgers already bind `AddString`, `AddGuiString`, `AddFormatText`, and `AddGuiFormatText` to `0x00401450/0x00401530/0x00401610/0x00401660` and describe their independent queue capacities. The `photo-game-task-draw-hud` ledger contains five `PhotoAsciiManagerTaskView::AddFormatText` REL32 relocations at offsets `0x130/0x165/0x1C5/0x215/0x24F`, all targeting `0x00401660`, plus the historical `g_PhotoAsciiManager @ 0x004A9F80` receiver. Other exact callers with `AddFormatText` decorations target `0x00401610`, providing an independent TH095-local caller partition. No TH08 semantic evidence is needed.

**Inferred.** The five photography HUD strings belong to the dedicated GUI text queue. The historical `PhotoAsciiManagerTaskView::AddFormatText` token is a proxy decoration whose return type and target match the GUI formatter; treating that token as canonical normal source routed HUD text into the wrong regular queue even though both callees link successfully. The maintainable correction is therefore caller routing to `AsciiManager::AddGuiFormatText`, not a rename or merge of the two formatter APIs.

**Unknown / bounded.** The original ZUN source-level identifier at the five call sites remains unknown, and this batch does not infer why the historical proxy used the `AddFormatText` spelling. It does not claim that all screen-space text belongs in the GUI queue: target-backed Main FPS, ReplayManager, ResultScreen, and SceneSelect callers continue to target the regular formatter. No runtime visual scenario is claimed for queue saturation, draw priority, or frame-by-frame HUD appearance; those runtime consequences remain a separate plane.

**Production / exact representation.** `src/PhotoGameTask.cpp` now uses a local VC7.1-compatible receiver/member compatibility pair around the five DrawHud calls. Reconstructed production expands it to `g_AsciiManager.AddGuiFormatText(...)`; DIFFBUILD retains the historical `g_PhotoAsciiManager.AddFormatText(...)` proxy surface. `TH095_MATCH_EXACT` remains frozen in `PhotoGameTaskExact.inl`. No shared header, AsciiManager field, queue size, function signature, HUD string, or exact relocation destination changes.

**Validation.** Focused replay rebuilt all 10 configured `PhotoGameTask.cpp` units and passed 10/10 exact with zero compiler-private label refresh. The first normal-production probe exposed a historical-toolchain source constraint rather than a semantic failure: VC7.1 rejects variadic macros, so that uncommitted helper form was immediately replaced by a receiver macro plus member-name macro; recovery confirmed no producer or temporary artifact remained. A command-local pinned-VC7.1 13.10.3077 normal compile then emitted a 35,493-byte Intel 80386 COFF `PhotoGameTask.obj`. Its relocation table contains exactly five references to `?AddGuiFormatText@AsciiManager...` at DrawHud offsets `0x130/0x165/0x1C5/0x215/0x24F` and zero references to canonical `?AddFormatText@AsciiManager...` from this TU.

Because the correction changes a cross-object callee, a fresh whole-product build immediately compiled all 88 production translation units and linked a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `62995315d68bb8d896b9994a968ede3168c3035a1d220fababedebe8b907b8a3`; this is production compile/link closure, not whole-image identity. The product map places regular `AddFormatText` at `0x0040CAD0`, GUI `AddGuiFormatText` at `0x0040CB20`, and `PhotoGameTaskView::DrawHud` at `0x00435850`. Linked-image disassembly shows its five HUD calls at `0x0043597F/0x004359B4/0x00435A14/0x00435A64/0x00435A9E` all resolving to `0x0040CB20`. The current-source cold aggregate exact gate then replayed all 88 manifest sources in four durable 22-source partitions and passed 205 + 158 + 191 + 142 = 696/696 configured units exact with zero private-label refresh. `git diff --check` passed. Runtime storage/scenario validation remains separate and unclaimed.

**Recovery / analysis state.** This transaction starts from committed handoff HEAD `f93406f4488acb29240c048a17d36f67d28abf5f` with zero staged/tracked unstaged changes and the same four pre-existing untracked paths excluded from staging. `.analysis/` began and remains 3,394,984 bytes; no current-session filesystem analysis root was created because fresh target evidence came through the registered read-only Ghidra provider. Two read-only Factory requests lost transport before returning command ids and received no evidence credit. The failed first normal compile used a command-local `/tmp` directory removed by its trap; mandatory recovery confirmed the intended tracked transaction as the only tracked change and no surviving compiler/Wine producer. The semantic phase remains active-incomplete.

**Next evidence route.** Continue the single-target relocation-proxy audit after checkpoint, but filter out the ABI proxy families already documented in `docs/KNOWLEDGE_BASE.md`. Prefer a mismatch where normal production can resolve successfully to two distinct canonical implementations, as happened here, because unresolved proxy-only symbols were largely closed by the 2026-09-09 ABI audit. Candidate names must be verified against target behavior and caller partition before editing. Otherwise rotate to a non-ANM resource/state or persistent/interpreter family with a new TH095-local producer plus independent consumer. Keep the established single-ended Unknown fields Unknown absent new evidence. The semantic phase remains active-incomplete.

### SEM-168 — share compact enemy secondary flags

**Scope.** Recovery at committed HEAD `c1610c4a10cf44b93e263b30fe0ecbf91f46b42b` found one coherent unfinished tracked transaction in `PhotoCamera.cpp`, `PhotoRuntime.cpp`, and `ecl/EnemyEclRuntimeView.hpp`. It promotes compact enemy `secondaryWord @ +0x2BF8` bits 6/7 into the shared view and replaces the two remaining photography-target raw bit-6 reads with the already-established `showPhotoMarker` meaning. The work was adopted only after fresh TH095 evidence confirmed the raw-test polarity. No meaning is assigned to bit 5.

**Observed.** Fresh target-attested `PhotoRuntimeView::CountPhotoTargets @ 0x004168D0` requires compact enemy `+0x2BF8` bit 6 clear before capture-AABB/ECL/item work. `PhotoCameraState::CountPhotoTargets @ 0x004339F0` independently requires the same bit clear before viewfinder containment, nearest-distance, and boss-rate accumulation. Fresh `UpdatePhotoMarkerPulse @ 0x00416770` reads bit 6, decrements the timer at `+0x2BFC`, and clears the bit on expiry. Fresh `PhotoEnemyManagerView::OnUpdate @ 0x00415970` independently tests adjacent bit 7 before attached-VM following.

**Corroborated.** SEM-042 already proved the bit-6 writer/lifetime protocol as `showPhotoMarker` through ECL opcode 143, the pulse timer, marker-VM consumer, and `PhotoRuntime` exclusion; SEM-041 similarly proved bit 7 as `freezeAttachedVm` through ECL opcode 158 and attached-VM following. The committed enemy/ECL normal views already use those names. PHOTO-094/SEM-143's later bit-6 `Unknown` statement was bounded to that bit-4 transaction and subsequent route prose copied it too broadly; fresh target evidence supports the stronger earlier proof. TH08 is not semantic authority here.

**Inferred.** `EnemyEclSecondaryBits` is the maintainable shared representation for the two proved controls. A set `showPhotoMarker` bit belongs to the marker-pulse interval and excludes the enemy from both audited target scans; `freezeAttachedVm` suppresses the established enemy-driven attached-VM follow update. This does not make the two bits one state machine.

**Unknown / bounded.** `secondaryWord` bit 5 and all unnamed bits other than 6/7 remain Unknown. The gameplay reason marker-visible enemies are excluded from capture/scoring is also Unknown. No serialization or persistence meaning is assigned to this in-memory word.

**Production / exact representation.** `src/ecl/EnemyEclRuntimeView.hpp` now exposes `EnemyEclSecondaryBits` with `showPhotoMarker` at bit 6 and `freezeAttachedVm` at bit 7 inside `secondaryWord @ +0x2BF8`; other bits stay anonymous. Normal `PhotoRuntime.cpp` and `PhotoCamera.cpp` read `TH095_ENEMY_ECL_SECONDARY_BITS(...).showPhotoMarker`. `TH095_MATCH_EXACT` and `DIFFBUILD` retain the historical raw `flags2 >> 6` expressions. Layout remains pinned at `+0x2BF4/+0x2BF8` with no added storage.

**Validation.** The seven direct shared-view users (`PhotoCamera`, `PhotoRuntime`, `EclDependencies`, `EclExtended`, `EclHelpers`, `EnemyShotAnm`, `ecl/EclRun`) replayed 48/48 configured exact units with zero private-label refresh, and command-local pinned VC7.1 13.10.3077 probes compiled all seven normal sources to Intel 80386 COFF. Because the header is shared, the cold aggregate gate replayed all 88 manifest sources in four durable partitions and passed 205 + 158 + 191 + 142 = 696/696 exact with zero refresh. The first request for partition three lost Factory transport before returning a command id and received no credit; recovery proved unchanged source state and no surviving producer before the durable retry. A fresh 88-TU whole-product build linked a PE32 i386 image with SHA-256 `8f596ef343b64fed4d9d3490b9680b1e81622c972a1d1990241f1a05b8eb9e16`; this is compile/link closure, not whole-image identity. Runtime-scenario validation remains separate and unclaimed.

**Recovery / analysis state.** The three tracked source paths were classified `recoverable-current-work`; user-owned `droid.resume.txt`, runtime experiments `config/runtime-scenarios.json` / `scripts/runtime-diff.py`, and unknown-origin generated-looking `EnemyManagerUpdate.i` remain preserved and excluded. `.analysis/` began at 3,394,984 bytes; no current-session filesystem analysis root or large artifact was created because target evidence came from the registered read-only provider. The semantic phase remains active-incomplete.

**Next evidence route.** Rotate away from compact bit 6/7. Treat post-SEM-143 route lines that list flags2 bit 6 as globally Unknown as stale; bit 5 and other unnamed secondary bits remain valid Unknowns unless a new distinguishing consumer appears. Prefer a distinct interpreter/state or persistent/resource family with a TH095-local producer plus independent consumer. The semantic phase remains active-incomplete.

### SEM-169 — share front-end asset queues

**Scope.** Rotate away from SEM-168 compact enemy flags into the front-end asynchronous scene-asset owner. The shared normal `SceneSelectControllerView` named only `selectionQueue`, `groupPreviewQueue`, and `scenePreviewQueue`, leaving `+0x6170..+0x62D7` and `+0x6368..+0x63AF` opaque even though `SceneSelectUpdate.cpp` and `SceneSelectAssets.cpp` already project those bytes as six additional `SceneValueQueue` instances. This transaction promotes the complete nine-queue owner into the shared normal header and replaces `UpdateMainMenu`'s production-only `&selectionQueue + N` indexing with direct member access. Frozen exact source and exact raw offsets are unchanged.

**Observed.** Fresh Factory-attested TH095 decompilation of `LoadSceneSelectionAssets @ 0x0044D0A0` shows nine contiguous 0x48-byte queue-shaped slots rooted at controller `+0x6128`. It reads selection requests from `+0x6128` with count `+0x6168`; publishes loaded-scene values to `+0x6170` with count `+0x61B0`; publishes group-preview data/size to `+0x61B8/+0x6200` with counts `+0x61F8/+0x6240`; publishes scene-preview data/size to `+0x6248/+0x6290` with counts `+0x6288/+0x62D0`; consumes group/scene preview requests from `+0x62D8/+0x6320` with counts `+0x6318/+0x6360`; and publishes loaded-group values to `+0x6368` with count `+0x63A8`. The next owner is the independent three-entry `stateHistory @ +0x63B0` with count `+0x63BC`.

Fresh `SceneSelectControllerView::UpdateMainMenu @ 0x00446A50` independently closes the other side of the protocol on the Game Start transition. After waiting for the asynchronous loader to stop, it drains and frees values from the group-preview-data queue at `+0x61B8` and scene-preview-data queue at `+0x6248`, then clears the counts at `+0x6240`, `+0x62D0`, `+0x6318`, `+0x6360`, `+0x63A8`, `+0x6168`, and `+0x61B0`. The target therefore treats the entire interval as one coherent queue owner rather than anonymous padding between three selected queues.

**Corroborated.** The TH095-local `SceneSelectUpdateView` already declares all nine queues in this exact order and asserts `selectionQueue @ +0x6128`, `loadedSceneQueue @ +0x6170`, `groupPreviewQueue @ +0x62D8`, `loadedGroupQueue @ +0x6368`, and `pendingTextureCount @ +0x63CC`. `SceneSelectAssets.cpp` independently declares the same nine-queue sequence and uses every queue in the producer/consumer protocol. SCENE-007 binds the complete selector state machine through `+0x63CC`; SCENE-009 binds the asynchronous loader. The shared representation therefore follows multiple TH095-local users and target offsets rather than importing adjacent-game layout.

**Inferred.** `SceneSelectControllerView +0x6128..+0x63AF` is maintainably represented as nine consecutive `SceneValueQueue` members: selection, loaded-scene, group-preview data, group-preview size, scene-preview data, scene-preview size, group-preview request, scene-preview request, and loaded-group. Their names describe the observed direction and payload role in TH095's loader/selector protocol; they do not claim original ZUN source identifiers.

**Unknown / bounded.** `SceneValueQueue` remains a 16-entry dword queue. Some observed channels carry 32-bit pointers and others carry packed integers or sizes, but this batch does not specialize the generic queue element type or infer universal ownership rules from those uses. The explicit frees in `UpdateMainMenu` establish cleanup for the two preview-data queues on that transition only. `stateHistory @ +0x63B0`, `pendingTextureCount @ +0x63CC`, and the later pending primary/secondary data/size arrays keep their existing meanings and are outside this representation change. Controller flag bit 4 remains Unknown.

**Production / exact representation.** Normal `src/SceneSelect.hpp` now names all nine queue members and adds offset assertions for the six newly exposed slots. Normal `src/FrontEndController.cpp` accesses those members directly when draining and resetting the Game Start queue state. `TH095_MATCH_EXACT` continues to include frozen `SceneSelectExact.hpp`, and the exact branch in `FrontEndController.cpp` retains its historical raw target offsets. No queue algorithm, element width, synchronization, capacity, controller size, or ABI changes.

**Validation.** Focused replay rebuilt `FrontEndController.cpp`, `SceneSelectAssets.cpp`, and `SceneSelectUpdate.cpp` and passed 4 + 1 + 1 = 6/6 configured exact units with zero compiler-private label refresh. Command-local pinned VC7.1 13.10.3077 probes compiled the same three normal-production sources to Intel 80386 COFF. Because `SceneSelect.hpp` is shared by twelve production translation units, the cold aggregate exact gate replayed all 88 manifest sources in four durable 22-source partitions and passed 205 + 158 + 191 + 142 = 696/696 configured units exact with zero private-label refresh. The first request for aggregate partition one lost Factory transport before returning a command id; mandatory recovery confirmed unchanged source state and no surviving replay/compiler/Wine producer before the durable retry.

A fresh whole-product build compiled all 88 production translation units with pinned VC7.1 and linked a PE32 i386 Windows GUI image with build-local SHA-256 `c959a03a00c3fda1b553fd8c01b43260ca3a420a4963d651f90cc4fd3fc30b82`; this is compile/link closure, not whole-image byte identity. `python3 scripts/ci.py` passed all 43 tests, `validate-tracking.py --require-target` remains 1,880 provisional / 697 source-present / 696 exact, and `git diff --check` passed. Two combined CI/tracking requests lost Factory transport before returning command ids and received no validation credit; the final split commands above are the durable validation results. Runtime-scenario validation is unchanged and unclaimed because this transaction only canonicalizes an already-observed queue layout and access path.

**Recovery / analysis state.** This transaction starts from committed SEM-168 checkpoint `74829906ed5bac320f34cd9a74ddf82a78cce5e1` with no staged or tracked unstaged work beyond the two source edits made here. The same four pre-existing untracked paths remain preserved and excluded from staging: user-owned `droid.resume.txt`, runtime experiments `config/runtime-scenarios.json` and `scripts/runtime-diff.py`, and unknown-origin generated-looking `EnemyManagerUpdate.i`. `.analysis/` remains 3,394,984 bytes; no current-session filesystem analysis root or large artifact was created. Fresh semantic evidence came from repository exact ledgers and the registered read-only target-attested Ghidra provider. The semantic phase remains active-incomplete.

**Next evidence route.** Rotate away from front-end/scene queue representation after checkpoint. Bounded negative routes in this campaign retained compact control bit 5, ANM VM bit 14, the TextRenderer RNG prefix, compact enemy tail `+0x4CA4..+0x4CAB` / `timer4cac`, and photo-score bits 5/17/18/19 as Unknown because no new distinguishing producer/consumer closed those protocols. The recent relocation-proxy sweep likewise found the major same-name hazards already canonicalized in normal production. Prefer a distinct persistent/ABI or non-ANM resource-lifetime family with a TH095-local producer plus independent consumer; replay/input or sound/PBG metadata should be revisited only when a new field-level reader appears. The semantic phase remains active-incomplete.


### SEM-170 — recover Best Shot raw-file ownership

**Scope.** Rotate from the front-end queue representation into a persistent/resource lifetime with independent TH095-local producer and cleanup paths. `ResultBestShotRecordView +0x70` was still named `componentData0`, even though the selected-scene loader treats it as the complete loose Best Shot file allocation. This transaction renames only that runtime owner to `rawFileData` in maintainable production and the matching PhotoStage partial layout. `pixelData @ +0x74`, the serialized 0x18-byte header, 0x50-byte comment, checksum protocol, file paths, and record validity flags are unchanged.

**Observed.** Fresh target-attested decompilation of `ResultSaveDataView::LoadBestShotForScene @ 0x00435E90` stores the return from loose-disk `FileSystem::OpenFile` directly at save owner `+0x3160 + recordIndex*0x78 + 0x70`. The target immediately copies six dwords (0x18 bytes) from that pointer into the staged record header and initializes its parse cursor to `rawFileData + 0x18`. It independently allocates `width * height * componentCount` bytes and stores that second allocation at record `+0x74`. Type 1 Best Shots decompress pixels from `rawFileData + 0x18`; the other observed format first copies the 0x50-byte comment from that same raw-file cursor, advances it, and decompresses the remaining bytes into `+0x74`. Successful checksum validation publishes `componentsLoaded @ +0x69` without freeing `+0x70`.

Fresh target `ResultSaveDataView::UpdateBestShotRecord @ 0x004299F0` independently closes the lifetime: it tests, frees, and clears record `+0x70`, then separately tests, frees, and clears decoded pixel owner `+0x74`, before clearing `componentsLoaded` and `valid`. Target save-data destructor `0x00435580` first releases the top-level score-file/decompressed payload owners and then calls this record cleanup across all 120 staged records. Thus the two Best Shot pointers are separate heap owners with different production roles, not interchangeable component slots.

**Corroborated.** FILE-002 establishes that `FileSystem::OpenFile @ 0x0041A960` with `loadFromDisk=TRUE` is the loose-file lane and returns an allocated file buffer. `LoadBestShotForScene` passes TRUE for `bestshot/bs_*.dat`, while the independently exact Scene preview path consumes only decoded `pixelData`. `WriteBestShotData` serializes the staged 0x18-byte header, comment, and compressed pixels rather than either runtime pointer. The owner interpretation is therefore TH095-local and does not rely on TH08 naming.

**Inferred.** `rawFileData` is the owning in-memory image of the Best Shot file used as the parser/decompression source; `pixelData` is a separately owned decoded image buffer. The raw owner survives a successful decode until the record is reset or the save-data owner is destroyed. Naming the field as raw file storage is narrower than the previous `componentData0` and matches both producer and cleanup semantics.

**Unknown / bounded.** The target evidence does not explain why the raw file allocation is retained after successful decompression instead of being freed immediately, and no additional lifetime or cache semantics are inferred. The field is runtime-only storage after the serialized record body; it is not claimed to be persisted. `unknown06a`, other score-entry reserved bytes, score-header `+0x0A/+0x0C`, and Best Shot format values retain their existing bounded meanings or Unknown status.

**Production / exact representation.** Normal `ScoreData.hpp` exposes `ResultBestShotRecordView::rawFileData @ +0x70`; normal `SceneBestShot.cpp` uses it as the OpenFile/parser base, normal `ResultScreen.cpp` releases it, and the compatible PhotoStage partial record uses the same name. `TH095_MATCH_EXACT` preserves historical `componentData0` because the frozen `SceneBestShotExact.inl` and `ResultScreenExact.inl` still address that token. The `+0x70/+0x74` layout assertions remain in place; no field size, record stride, allocator, serialization byte, or ABI changes.

**Validation.** The first focused exact attempt intentionally exposed the representation boundary: changing the shared member name unconditionally made frozen `SceneBestShotExact.inl` fail to compile on the historical `componentData0` token. No exactness credit was taken from that failed command. After adding the exact-only historical spelling, focused replay of `PhotoStage.cpp`, `ResultScreen.cpp`, `SceneBestShot.cpp`, and `ScoreData.cpp` passed 6 + 24 + 1 + 1 = 32/32 configured units exact with zero private-label refresh. Command-local pinned VC7.1 13.10.3077 normal probes compiled the same four sources to Intel 80386 COFF. Because `ScoreData.hpp` is shared persistent ABI, the cold aggregate gate replayed all 88 manifest sources in four durable partitions and passed 205 + 158 + 191 + 142 = 696/696 exact with zero private-label refresh. Two requests for the third partition were lost before returning Factory command ids and received no credit; recovery confirmed unchanged source state and no active producer before the durable explicit-source retry. A fresh whole-product build compiled all 88 production TUs and linked a PE32 i386 image with build-local SHA-256 `88e73d00e521aa1c8160f30d61828d98f38704d118bbc1605335fd7b97b2e315`; this is production compile/link closure, not whole-image identity. Runtime-scenario validation is separate and unclaimed.

**Recovery / analysis state.** This transaction starts from committed continuation HEAD `d838eb1e1ffc36fa9a4d7e41cb8394434aa730df`. The four pre-existing untracked paths remain preserved and excluded from staging. `.analysis/` began at 3,394,984 bytes and no current-session filesystem analysis root or large retained artifact was created; fresh target evidence came through the registered read-only attested Ghidra provider. The semantic phase remains active-incomplete.

**Next evidence route.** Rotate away from Best Shot buffer ownership after checkpoint. Prefer another independent non-ANM resource lifetime or persistent/ABI field with a fresh reader/writer relation. Do not revisit score-header `+0x0C`, replay reserved bytes, Sound/PBG writer-only metadata, compact-enemy plateaus, ANM bit 14, TextRenderer RNG prefix, or photo-score bits 5/17/18/19 without a new TH095-local distinguishing consumer. The semantic phase remains active-incomplete.


### SEM-171 — recover ReplayManager header allocation ownership

**Scope.** Continue from the committed SEM-170 Best Shot lifetime checkpoint and rotate to the independent replay persistent/resource owner. `ReplayManager +0x04` was still named `fileHeader`, which describes the typed bytes at the allocation root but hides the fact that the manager destructor owns that pointer in every mode. This transaction renames only the normal-production field to `ownedFileHeader`. Replay header bytes, replay modes, payload transforms, stream layouts, allocator calls, file-source selection, and exact-facing source remain unchanged.

**Observed.** Fresh Factory-attested TH095 decompilation of `ReplayManager::Initialize @ 0x004342A0` shows record mode allocating exactly 0x24 bytes and storing the result at manager `+0x04`, zeroing all nine dwords, and writing replay magic/version/game-version fields through that pointer. Fresh `ReplayManager::LoadReplay @ 0x00435130` proves two different producer shapes for the same owner. In the loose `replay/%s` lane, `ReplayFile::Read(0x24)` allocates the header stored at `+0x04`; a second `ReplayFile::Read(compressedPayloadSize)` allocation supplies the encrypted/compressed payload and is freed after decryption/decompression. In the archive-backed lane, `FileSystem::OpenFile(path, ..., FALSE)` stores the complete archive-entry allocation at `+0x04`, and the compressed payload is an interior pointer at `ownedFileHeader + 0x24` rather than a second allocation.

Fresh target `ReplayManager::~ReplayManager @ 0x004344F0` independently closes the lifetime. It always frees the allocation at manager `+0x08` when present, frees `+0x0C` only in record mode because loaded FPS data is an interior view into the decompressed input allocation, and then tests/frees manager `+0x04` regardless of replay mode. Fresh `WriteReplay @ 0x00434A90` mutates compressed/decompressed sizes and USER offset through the same `+0x04` root and writes exactly the first 0x24 bytes as the persistent file header. The field is therefore both the typed header base and an owning allocation root.

**Corroborated.** Canonical `ReplayFile::Read @ 0x0041B020` allocates exactly the requested byte count with `malloc`, fills it with `ReadFile`, and returns that owning pointer. FILE-002/FILE-007 establish that `FileSystem::OpenFile(..., FALSE)` selects the PBG archive-only lane and returns an allocated decompressed entry image. REPLAY-005/006/007 independently establish the 0x24 replay header and record/load/write protocol. These are all TH095-local observations; no TH08 field naming is used as authority.

**Inferred.** `ownedFileHeader` is the narrow maintainable name because all observed values point at a `ReplayFileHeader` at offset zero and the manager is responsible for freeing that root. The name deliberately describes ownership without claiming a fixed allocation extent: record mode and loose-file load can own a standalone 0x24-byte header, while archive-backed load owns a larger allocation whose first 0x24 bytes are the header and whose payload follows in-place.

**Unknown / bounded.** The original ZUN identifier is unknown. This batch does not infer an explicit stored allocation size or source discriminator for `+0x04`; the current mode/archive globals and producer path determine the backing shape. It does not add validation for replay magic/version/game version, header reserved bytes, compressed/decompressed sizes, user-data offset, or malformed archive/loose-file boundaries. Those trust hazards remain as documented in REPLAY-020. Replay header reserved ranges and replay/input unknown bytes remain Unknown.

**Production / exact representation.** Normal `ReplayManager.hpp` exposes `ReplayManager::ownedFileHeader @ +0x04`, and normal `ReplayManager.cpp` uses that name in record initialization, load, write, and destruction. `TH095_MATCH_EXACT` retains `ReplayManager::fileHeader`, and `ReplayManagerExact.inl` remains frozen. The pointer type, `ReplayManager` size 0x12C, all later member offsets, serialized header size 0x24, and allocator/free behavior are unchanged.

**Validation.** Focused canonical replay rebuilt `ReplayManager.cpp` and passed all 12/12 configured units exact with zero compiler-private label refresh. A command-local pinned VC7.1 13.10.3077 normal probe emitted an Intel 80386 COFF `ReplayManager.obj`. Because `ReplayManager.hpp` is shared, the current-source cold aggregate gate was rerun across all 88 manifest sources. The first unpartitioned request executed long enough to rebuild matching objects but the Factory transport timed out without a command id or final result, so it received no validation credit. Four durable explicit-source partitions then passed 205 + 158 + 191 + 142 = 696/696 configured units exact with zero private-label refresh. One pre-execution network failure before partition three likewise received no credit; a process check showed no surviving replay/compiler/Wine producer before the durable retry.

A fresh whole-product build compiled all 88 production translation units with pinned VC7.1 and linked/verified a 780,288-byte PE32 i386 Windows GUI image with build-local SHA-256 `a56737e848b8d07723884f3431a82896a59d9c499d05f5c6591d209ad6f2ee92`; this is production compile/link closure, not whole-image byte identity. Target-independent CI passed all 43 tests, `validate-tracking.py --require-target` remains 1,880 provisional / 697 source-present / 696 exact, and `git diff --check` passed. Runtime storage identity, runtime scenarios, and portable-runtime validation remain separate and unclaimed.

**Recovery / analysis state.** Recovery after the browser interruption found committed SEM-170 checkpoint `16b75eff7a0bd14affbbda8eebcab55f2482d22d` plus exactly two tracked `recoverable-current-work` paths, `src/ReplayManager.cpp` and `src/ReplayManager.hpp`, containing this coherent owner rename; there were no staged changes. The same four pre-existing untracked paths remain preserved and excluded: user-owned `droid.resume.txt`, runtime experiments `config/runtime-scenarios.json` and `scripts/runtime-diff.py`, and unknown-origin generated-looking `EnemyManagerUpdate.i`. `.analysis/` started and remains 3,394,984 bytes; no current-session filesystem analysis root or retained large artifact was created because fresh semantic evidence came from the registered read-only target-attested Ghidra provider and repository-local exact sources.

**Next evidence route.** Rotate away from the replay-header owner after checkpoint. Prefer a different owner/lifetime, interpreter/state, or persistent/ABI family with a TH095-local producer plus independent consumer. A replay follow-up may examine the mode-dependent `fpsData` owned-allocation versus interior-view lifetime only if it yields a maintainability improvement beyond the already correct destructor guard; do not rename for novelty. Keep replay reserved bytes, ReplayScanWorker `unknown010`, score-header reserved fields, Sound/PBG writer-only metadata, ANM bit 14, TextRenderer RNG prefix, compact-enemy plateaus, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.


### SEM-172 — expose the front-end replay column cursor

**Scope.** Rotate from the ReplayManager allocation owner into the independent shared front-end ABI. `FrontEndControllerUpdateView` already typed the first `ResultScreenReplayCursor @ +0x20` but then represented the complete `+0xF8..+0xBF3` region as `unknown00f8`, even though committed TH095 evidence identifies another cursor subobject at `+0xF8`. This transaction changes only the normal-production layout projection: it exposes the second cursor as `replayColumnCursor @ +0xF8` and moves the remaining anonymous span to `unknown01d0`. Runtime behavior, object size, later member offsets, replay-browser logic, and exact-facing source are unchanged.

**Observed.** Fresh Factory-attested TH095 decompilation of `FrontEndLifecycleView::FrontEndLifecycleView @ 0x00445440` independently initializes two cursor-shaped subobjects. The first begins at owner `+0x20`: it clears `current`, initializes `wraps = 1`, and sets `count = 999`. The second repeats the same cursor initialization at owner `+0xF8`. The constructor then initializes twelve more cursor-shaped scene-selection records beginning at `+0x1D0`, matching the already established 0xD8 `ResultScreenReplayCursor` size and FRONT-008 construction chronology.

Fresh `ReplayBrowserView::Update @ 0x0044DCA0` supplies the state-specific meaning. On entry it configures the `+0x20` cursor for four pages and seeds its current value from `g_ReplayBrowserSelection / 20`; it configures the `+0xF8` cursor for twenty entries and seeds its current value from `g_ReplayBrowserSelection % 20`. During navigation, the target independently saves/moves/tests both cursor objects and combines `row.current * 20 + column.current` to select one of eighty replay slots. Fresh `SceneSelectControllerView::Draw @ 0x00452630` provides an independent consumer: in replay state 3 it uses owner `+0x20.current` to choose the twenty-entry page and compares owner `+0xF8.current` against `index % 20` to choose the highlighted row.

**Corroborated.** Normal `ReplayBrowserView` already types these physical objects as `rowCursor @ +0x20` and `columnCursor @ +0xF8`, with compile-time offset assertions. FRONT-001 and FRONT-003 independently record the draw-side page/selection reads; FRONT-008 records the constructor's two cursor subobjects before the twelve scene cursors. The common `ResultScreenReplayCursor` definition is 0xD8 bytes, so the second object ends exactly at `+0x1D0`, preserving the following scene-cursor region and `SceneAnmVmIdArray @ +0xBF4`.

**Inferred.** `replayColumnCursor` is deliberately state-qualified in the generic update projection. The physical subobject is a second reusable cursor, but the current TH095-local behavioral evidence assigns it a concrete role only in Replay Browser state. The first `+0x20` member remains named simply `cursor`: normal `UpdateMainMenu` also drives that same object as the six-entry title-menu cursor, so a global `rowCursor` spelling would overstate replay-only semantics.

**Unknown / bounded.** This batch does not reinterpret the `+0x1D0..+0xBF3` region beyond moving the anonymous span start past the proven second cursor. In particular, owner `+0xBF0` remains state-dependent: the replay-browser projection calls it `selectedReplayIndex`, while the draw-side scene-summary projection consumes the same physical dword as `selectedScoreEntry`. No universal field name is claimed. The second cursor's original ZUN identifier and any non-Replay-Browser use remain unknown. No replay format, persistent data, input protocol, or runtime scenario changes are introduced.

**Negative oracle / representation correction.** The first normal-production attempt renamed the shared `+0x20` member to `rowCursor` and exposed `+0xBF0` as `selectedReplayIndex`. Exact replay still passed because the exact branch preserved its historical projection, but the normal VC7.1 compile correctly failed at the real `UpdateMainMenu` consumers of `view->cursor`. Inspecting those consumers falsified the over-specific global name: `+0x20` is also the wrapped six-entry title-menu cursor. The final source therefore keeps `cursor @ +0x20`, exposes only `replayColumnCursor @ +0xF8`, and leaves the state-dependent `+0xBF0` meaning inside the bounded anonymous region.

**Production / exact representation.** Normal `FrontEndControllerUpdateView` now lays out `cursor @ +0x20`, `replayColumnCursor @ +0xF8`, and `unknown01d0[0xA24]` before the existing `vmIds @ +0xBF4`. A new compile-time assertion pins `replayColumnCursor` to `+0xF8`; all existing assertions through `entryMode @ +0x6124` remain unchanged. `TH095_MATCH_EXACT` retains the historical `cursor` plus `unknown00f8[0xAFC]` source shape, so no exact-facing token/layout experiment is rewritten.

**Validation.** After the representation correction, canonical replay of all four configured `FrontEndController.cpp` units passed 4/4 exact with zero private-label refresh. A command-local pinned VC7.1 13.10.3077 normal-profile probe compiled the same source to an Intel 80386 COFF object, and `git diff --check` passed. Because this is a TU-local projection and not a shared header/PCH/owner-layout definition consumed across translation units, this private checkpoint does not replay the full 696-unit accepted set or whole-product gate; those remain milestone/final-handoff checks rather than per-transaction churn.

**Recovery / analysis state.** The batch began from committed HEAD `923b0f50ac57195c28c21c1024703f5841a7031d` with no staged or tracked-unstaged work and the four documented pre-existing untracked paths preserved outside staging. `.analysis/` remains the campaign baseline 3,394,984 bytes; no current-session filesystem analysis root or retained large artifact was created because fresh semantic evidence came from the registered read-only target-attested Ghidra provider. Intermittent Factory transport connection failures occurred before several read-only shell/validation calls; calls without a durable command/result received no evidence or validation credit.

**Next evidence route.** Rotate away from front-end replay cursors after checkpoint. Prefer an independent historical-runtime, non-ANM resource-lifetime, primary/sibling interpreter-state, or persistent/ABI family with a TH095-local producer plus independent consumer. Keep the state-dependent front-end `+0xBF0` alias, `ReplayInputSource::unknown02a/unknown036`, ReplayScanWorker `unknown010`, score/replay reserved words, SoundPlayer/PBG writer-only metadata, ANM bit 14, TextRenderer RNG prefix, compact-enemy plateaus, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.


### SEM-173 — bind high-ECL ANM handle slot producer and consumer

**Scope.** Rotate from the post-SEM-172 owner/cursor work into the primary ECL interpreter and close one missed binding to an already-proven compact-enemy owner. High ECL opcodes 151 and 152 still addressed `Enemy +0x2D4 + slot*4` through raw byte arithmetic even though SEM-016 established `Enemy::anmHandles[2] @ +0x2D4` from independent TH095-local consumers. This transaction changes only the normal-production representation of those two sibling opcode paths: opcode 151 publishes a newly spawned ANM VM handle into the selected slot, and opcode 152 reads the selected slot to resolve a VM and deliver an interrupt. Opcode values, operand decoding, slot arithmetic, array extent, VM-id representation, control flow, and exact-facing source remain unchanged.

**Observed.** Fresh Factory-attested TH095 decompilation of `EclManager::RunEcl @ 0x00408E70` shows target switch case `0x97` (decimal opcode 151) calling the ANM spawn path at `0x00445060` and storing its returned four-byte VM id at `enemy + resolvedSlot*4 + 0x2D4`. The immediately related target case `0x98` (decimal opcode 152) reads the four-byte value from the same indexed address, passes it to `AnmManager::GetVm @ 0x00445110`, and, when a VM is found, writes the requested interrupt to that VM's interrupt field at `+0x22E`. Neither case performs a slot bounds check in the target, so this reconstruction does not add one.

Fresh target decompilation independently reconfirmed `UpdatePlayerProximityAndMarker @ 0x00413AA0` reading enemy `+0x2D4` through the same exact `AnmManager::GetVm` resolver, and `UpdateEnemyMarkerVms @ 0x00413B90` resolving both enemy `+0x2D4` and `+0x2D8`. `AnmManager::GetVm @ 0x00445110` walks the manager VM list and compares the supplied integer against each VM's four-byte id. These are TH095-local consumers, not adjacent-game semantic authority.

**Corroborated.** The canonical `Enemy` layout declares `i32 anmHandles[2]` and asserts `offsetof(Enemy, anmHandles) == 0x2D4`, followed immediately by `mainEclContextStorage @ +0x2DC`. SEM-016 already established the two dwords as ANM VM handles from the extended-ECL consumers but deliberately left their producer/reset/lifetime protocol outside that batch. The current high-ECL cases supply one previously unbound producer plus its sibling direct consumer. Existing `EclExtended.cpp` continues to consume the same canonical slots, so the normal interpreter and extended callback lanes now share one owner representation.

**Inferred.** `Enemy::anmHandles[slot]` is the maintainable owner expression for the indexed storage used by opcodes 151/152. This is a representation/owner binding, not a claim that either slot has one universal gameplay role. The opcode behaviors can be described narrowly as spawn-into-selected-ANM-handle-slot and interrupt-selected-ANM-handle-slot; the original ZUN opcode names remain unknown.

**Unknown / bounded.** This batch does not infer a valid script-level slot domain beyond the physically declared two-slot owner, because the target cases themselves do not validate the resolved index. It does not establish every producer, reset, deletion, or lifetime edge for the pair, does not assign distinct semantic names to slots 0 and 1, and does not convert the POD `i32` storage into non-trivial `AnmVmId` members. Those stronger lifetime and script-contract claims remain outside the evidence. Other ECL anonymous layout prefixes and single-ended fields remain Unknown unless a new TH095-local producer/consumer relation distinguishes them.

**Production / exact representation.** `src/ecl/EclRunHigh.inl` now routes the selected slot through `TH095_ENEMY_ANM_HANDLE_SLOT`. Normal production expands that accessor to `enemy->anmHandles[slot]`; `TH095_MATCH_EXACT` retains the historical `reinterpret_cast<u8 *>(enemy) + 0x2D4 + slot*4` lvalue shape. No shared header, object layout, PCH-visible declaration, ABI, serialized format, or runtime storage changes.

**Validation.** Focused canonical replay `python3 scripts/replay-exact-units.py --source src/ecl/EclRun.cpp` rebuilt the one configured `RunEcl` unit and passed 1/1 exact with zero compiler-private label refresh. A command-local normal-production probe using pinned VC7.1 13.10.3077 and the canonical `/MT /EHsc /Gs /DNDEBUG /Zi /Gy /GF /Oi /Gr /Od /Ob1 /I src/ecl /I src` profile emitted a 77,946-byte Intel 80386 COFF `EclRun.obj`; the temporary object/PDB directory was removed by the same command. `git diff --check` passed. This transaction is translation-unit-local and changes no shared owner/layout declaration, so cold aggregate exact and whole-product gates are deferred to the committed campaign milestone/final handoff rather than replayed before this private checkpoint.

**Recovery / analysis state.** The campaign began from committed HEAD `81d839f432db638a702a3a212769abb1c723c7d5` with no staged or tracked-unstaged changes and the four documented pre-existing untracked paths preserved outside staging. `EnemyManagerUpdate.i` remains unknown-origin generated-looking state; `droid.resume.txt` remains explicit user/external state; `config/runtime-scenarios.json` and `scripts/runtime-diff.py` remain runtime experiments. Their hashes were rechecked unchanged before this batch. `.analysis/` started at 3,394,984 bytes; no current-session `.analysis/gpt-web/` root or retained large artifact was created because target evidence came from the registered read-only attested Ghidra provider. One pre-edit read-only Factory transport request failed before receiving a command id; repository status was rechecked unchanged and the failed request received no evidence credit.

**Next evidence route.** After checkpoint, rotate away from this ANM-handle pair rather than mining SEM-016 again. Prefer a distinct historical-runtime or non-ANM resource-lifetime owner, a persistent/ABI field with a new reader/validator, or another primary/sibling interpreter-state protocol whose producer and independent consumer are still represented weakly. Keep replay/input reserved bytes, `ReplayScanWorker::unknown010`, score-header reserved fields, Sound/PBG writer-only metadata, ANM VM bit 14, the TextRenderer RNG prefix, compact-enemy single-ended tails/control gaps, and photo-score bits 5/17/18/19 Unknown absent new distinguishing TH095-local evidence. The semantic phase remains active-incomplete.


### SEM-174 — bind high-ECL deferred-shot cadence producers

**Scope.** Continue the adversarial resume audit from SEM-173 and test normal-production raw offsets in the target-high interpreter against already-established TH095-local owners. SEM-024 proved the compact deferred-shot cadence family from the periodic update consumer and the high-ECL producer behavior, but its maintainable source edit was confined to `EnemyShotAnm.cpp`; target-high opcodes 95 and 96 still addressed compact enemy `+0x2BC8/+0x2BCC` directly in normal production. This transaction binds only those two missed producer paths to a translation-unit-local `Th095EnemyShotCadenceView`. It does not revise SEM-024's meaning, rename either opcode, reinterpret the cached shot instruction at `+0x2B9C`, or change the deferred-shot control bit.

**Observed.** Fresh Factory-attested TH095 decompilation of `EclManager::RunEcl @ 0x00408E70` reconfirms target case `0x5F` (decimal 95) writing its resolved integer operand to enemy `+0x2BC8`; when nonzero it resets the `ZunTimer` rooted at `+0x2BCC`. Target case `0x60` (decimal 96) writes the same interval dword and, when nonzero, obtains a bounded TH095 RNG value modulo that interval and publishes it into the timer while restoring the timer's reference/count representation. These are producer paths for the same two physical fields, not independent storage.

Fresh target decompilation of `Enemy::UpdateShotAndAnm @ 0x00413030` independently reads `+0x2BC8`, advances the timer rooted at `+0x2BCC`, compares its current count against the configured interval, dispatches `DispatchShotInstruction @ 0x00412670` with cached instruction storage at enemy `+0x2B9C` when due, and resets the timer afterward. The consumer therefore distinguishes the interval dword from the timer and closes the producer/consumer relation with TH095-local evidence.

**Corroborated.** SEM-024 already established `EnemyShotCadenceView` for the same compact offsets from `UpdateShotAndAnm`, exact RunEcl producers, and the compact `PhotoEnemyView`. SEM-113 later found and bound an independent scheduled-call reset producer at the same `shootIntervalFrames @ +0x2BC8`, demonstrating that post-SEM-024 resume audits may legitimately find missed representation consumers/producers without reopening the semantic identity. Current repository history shows the target-high 95/96 raw expressions predate those later maintainable owner bindings and had not themselves been canonicalized.

**Inferred.** The maintainable normal-production owner for the target-high writes is the same compact deferred-shot cadence state: `shootIntervalFrames @ +0x2BC8` and `shootIntervalTimer @ +0x2BCC`. Opcode 95 initializes the timer at the beginning of the configured cadence, while opcode 96 seeds the timer to an RNG-selected value within that interval. Those descriptions are data-flow observations; no original ECL mnemonic or higher-level gameplay intent is claimed.

**Unknown / bounded.** This batch does not assign names to opcodes 95/96, does not infer script conventions for zero or negative interval values beyond the target's observed `!= 0` producer-side tests and the update consumer's `> 0` gate, and does not claim the cadence survives every enemy lifecycle transition. It does not reinterpret the cached `0x2C` shot instruction, the defer-shot control bit, neighboring compact fields, or any randomization policy beyond the observed bounded RNG call. Other single-ended compact fields remain Unknown.

**Production / exact representation.** `src/ecl/EclRunHigh.inl` adds a translation-unit-local `Th095EnemyShotCadenceView` with compile-time offset checks for `shootIntervalFrames @ +0x2BC8` and `shootIntervalTimer @ +0x2BCC`. Normal cases 95/96 in `src/ecl/EclRunTargetHigh.inl` address those named members. `DIFFBUILD` and `TH095_MATCH_EXACT` retain the historical raw pointer expressions verbatim, including the exact-only timer-initialization helper. No shared header, physical layout, PCH-visible declaration, persistent format, ABI, opcode number, operand decoding, or control flow changes.

**Validation.** Focused canonical replay `python3 scripts/replay-exact-units.py --source src/ecl/EclRun.cpp` passed the configured `ecl-manager-run-ecl` unit 1/1 exact with zero compiler-private label refresh. A command-local normal-production probe using pinned VC7.1 13.10.3077 and the repository profile `/MT /EHsc /Gs /DNDEBUG /Zi /Gy /GF /Oi /Gr /Od /Ob1 /I src/ecl /I src` emitted a 77,946-byte Intel 80386 COFF object; its temporary object/PDB directory was removed by the same command. `git diff --check` passed. Because this transaction is private to the one RunEcl translation unit and changes no shared owner/layout declaration, cold aggregate exact and whole-product gates are deferred to the campaign milestone after checkpoint.

**Recovery / analysis state.** This transaction began from committed SEM-173 HEAD `97feb59fb782c56aaf581e18da25d289713da9cb` with no staged or tracked-unstaged changes and the same four pre-existing untracked paths preserved outside staging. No current-session `.analysis` artifact was created; target evidence came from the registered read-only Ghidra provider. The campaign `.analysis/` baseline remains 3,394,984 bytes with no retained current-session large artifact.

**Next evidence route.** After checkpoint, rotate away from deferred-shot cadence and the ANM-handle pair. The resume audit has already falsified several tempting raw-debt routes as exact/DIFF compatibility surfaces rather than live normal gaps: SceneSelect Best Shot records, FrontEnd shared asset queues, compact marker-pulse timer, Player position, compact movement bounds, and historical relocation proxies. A later batch should prefer a distinct persistent/ABI, historical-runtime, non-ANM resource lifetime, or interpreter/state family with a new TH095-local producer plus independent consumer. Keep `+0x2CA4/+0x2CA8`, score-header reserved fields, ReplayScanWorker `unknown010`, replay/input reserved bytes, Sound/PBG writer-only metadata, ANM bit 14, TextRenderer RNG prefix, compact tail/control single-ended storage, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.

### SEM-175 — align PhotoCard with shared photo runtime gates

**Scope.** Continue the post-SEM-174 resume audit outside the recently touched high-ECL families and test shared photo-runtime consumers that were not included in the original owner repairs. `PhotoCardInfoView::OnUpdate` still decoded `g_RuntimeGlobalStateOwner +0xFC` bits 2 and 10 through raw shifts in normal production, while `OnDraw` decoded bit 2 the same way. SEM-055/SEM-056 already establish those physical bits as `gameplayLoadActive` and `photoTransitionActive` from independent TH095-local producers and consumers, but the PhotoCard callbacks were not part of either maintained representation batch. This transaction binds only those missed PhotoCard consumers to a bounded local owner view. It does not reinterpret either shared bit, change PhotoCard state/VM behavior, or rename any unresolved neighboring flag.

**Observed.** Fresh Factory-attested TH095 decompilation of `PhotoCardInfoView::OnUpdate @ 0x00408C60` reads `DAT_004BDEC8 + 0xFC` bit 2 first and returns the chain-continue value without calling `PhotoCardInfoView::Update @ 0x004089F0` while it is set. If bit 2 is clear, the same callback independently reads bit 10 and likewise suppresses the update while that bit is set. Fresh `OnDraw @ 0x00408CB0` reads the same owner bit 2 and suppresses the otherwise trivial `Draw @ 0x00408C50` call while gameplay loading is active.

Fresh producer observations keep the two gates distinct. `PhotoGameTaskView::Load @ 0x00417D20` sets bit 2 on the same `0x004BDEC8 + 0xFC` owner before waiting for capture/ANM work and clears it only on the successful load path. `EclExtended::EnablePhotoTransition @ 0x00414430` sets bit 10 while starting the paired Background transition VMs/sound, and `DisablePhotoTransition @ 0x004144E0` clears that bit while running the matching end sequence. Thus the PhotoCard reads join two already independent TH095-local state lifetimes rather than defining a new generic pause bit.

**Corroborated.** SEM-055 established shared bit 2 `gameplayLoadActive` from the asynchronous PhotoGameTask load lifetime and subsystem consumers. SEM-056 established shared bit 10 `photoTransitionActive` from the ECL transition producer/clearer and Background, Bullet, Effect, and PhotoGame consumers; that record explicitly did not claim exhaustive consumer coverage. Later SEM-102 demonstrated the same resume-audit pattern by finding PhotoItem as another missed consumer of established shared flags. The current source/history search found no later semantic record that bound PhotoCard's raw bit-10 read, so this is a genuine representation gap rather than a repeated batch.

**Inferred.** PhotoCard participates in the shared gameplay-state protocol at a narrow callback boundary: gameplay loading suppresses both its update and draw callbacks, while the ECL-driven photo transition suppresses update only. Those are observed per-callback effects of the established owner fields; this batch does not generalize either bit into a universal UI freeze policy.

**Unknown / bounded.** Shared bit 8 and all other unresolved task/global-state bits remain Unknown. This batch does not assign semantics to `PhotoCardInfoView::unknown000`, `unknown050`, its VM ids, or any stage-state bits adjacent to `firstCaptureFrame`. It does not claim that PhotoCard drawing is intentionally allowed during `photoTransitionActive` for a particular visual-design reason; the target simply does not test bit 10 in `OnDraw`. No runtime visual scenario is claimed.

**Production / exact representation.** Normal `src/PhotoCardInfo.cpp` now gives its local `PhotoCardGameTaskView` a layout-preserving bitfield with only bit 2 `gameplayLoadActive` and bit 10 `photoTransitionActive` named; bits 0..1, 3..9, and 11..31 remain anonymous. Normal `OnUpdate`/`OnDraw` consume the named fields directly. `DIFFBUILD` retains the historical raw `u32 flags` projection and shift tests, while `TH095_MATCH_EXACT` continues to select the frozen `PhotoCardInfoExact.inl` at the source-file boundary. Physical storage, owner address, callback signatures, branch order, and control flow are unchanged.

**Validation.** Focused canonical replay `python3 scripts/replay-exact-units.py --source src/PhotoCardInfo.cpp` rebuilt all ten configured PhotoCard units and passed 10/10 exact with zero compiler-private label refresh. A command-local normal-production probe reused the manifest `/MT /EHsc /Gs /DNDEBUG /Zi /Gy /GF /Oi /Gr /Od /Ob1 /I src` profile with pinned VC7.1 13.10.3077 and emitted a 26,089-byte Intel 80386 COFF object; its temporary object/PDB directory was removed by the same command. `git diff --check` passed. Because this is a private translation-unit representation change with no shared header/layout/PCH/owner mutation, cold aggregate exact and whole-product gates are deferred to the next committed campaign milestone/final handoff.

**Recovery / analysis state.** The transaction began from committed handoff HEAD `11258f72ff80280721e9d6eb1362dad173d25f39` with no staged or tracked-unstaged changes and the four documented pre-existing untracked paths preserved outside staging. `.analysis/` began at 3,394,984 bytes and no current-session analysis artifact was created; fresh semantic evidence came through the registered read-only target-attested Ghidra provider. Several read-only Factory requests during earlier negative routing lost transport before returning durable command ids or results; each received zero evidence credit and live repository status was rechecked before continuing.

**Next evidence route.** After checkpoint, rotate away from shared photo-runtime gates and PhotoCard. Prefer an independent persistent/ABI, historical-runtime, non-ANM resource-lifetime, replay/input, or sibling-interpreter family with a new TH095-local producer plus independent consumer/cleanup edge. Keep shared bit 8, FrontEnd controller bit 4, `ReplayScanWorker::unknown010`, replay/input reserved storage, score-header reserved fields including `+0x0C`, Sound/PBG writer-only metadata, ANM VM bit 14, the TextRenderer RNG prefix, compact enemy single-ended tail/control fields, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.

### SEM-176 — bind target-high BulletInf ANM owner reads

**Scope.** Rotate away from SEM-175 shared photo-state gates into the independent ECL-to-resource owner boundary. The TH095 BulletInf layout and SEM-017 already establish manager `+0x27C5B0` as the slot-six `bullet.anm` `AnmLoaded *`/ANM spawner owner, but three target-high `RunEcl` spawn paths still recovered that pointer through raw `TH095_ECL_BULLET_MANAGER + 0x27C5B0` arithmetic in normal production. This transaction binds only those missed interpreter-side owner reads to an EclRun-local `PhotoBulletManagerView::anmSpawner` projection. It does not change the BulletInf lifetime, ANM slot number, spawned scripts, VM handle storage, or any ECL opcode identity.

**Observed.** Fresh Factory-attested TH095 decompilation of `PhotoBulletManagerView::Initialize @ 0x00404E00` calls the ANM preload/load routine at `0x004432E0` with slot `6` and `"bullet.anm"`, then stores the returned pointer into BulletInf `+0x27C5B0`; failure branches through the existing error path. Fresh `EclManager::RunEcl @ 0x00408E70` independently reads `DAT_004BDD98 + 0x27C5B0` in the photo-card start path before calling `CreateVmAtWorld @ 0x00445060` for script `0xD2`, and again in the photo-pulse path before spawning script `0x125`. The exact target-high body contains the same owner read for both historical photo-pulse source branches, so all three maintained call sites resolve the identical BulletInf-owned pointer.

**Corroborated.** `PhotoBulletManagerView` in the canonical Bullet manager reconstruction asserts `anmSpawner @ +0x27C5B0`; its initializer, bullet spawn/update paths, PhotoCamera, PhotoGame, and EclExtended all consume the same field. ABI-033 independently identifies BulletInf `+0x27C5B0` as the `AnmLoaded *` loaded from slot six, and PHOTO-054 establishes it as part of the exact `0x27C5B8` BulletInf owner. SEM-017 canonicalized the same physical member for the extended-ECL callback family only; repository history and semantic records contain no later batch that bound the target-high `RunEcl` raw reads, so this is a missed sibling-interpreter representation rather than a repeated owner recovery.

**Inferred.** The narrow maintainable role of BulletInf `+0x27C5B0` at these sites is the manager-owned ANM spawner used to create photography VMs. The ECL callbacks borrow that loaded ANM resource; they do not allocate, replace, or release the owner. This source representation therefore expresses an already-established ownership edge without assigning higher-level script meaning to the calling opcodes.

**Unknown / bounded.** This batch does not infer why the photo-card path uses script `0xD2`, why the pulse path uses `0x125`, or whether every BulletInf ANM consumer has identical lifetime requirements. It does not merge the separate enemy-runtime ANM owner at `+0x4DF8`, change VM handle ownership, reinterpret ANM preload-slot path bytes, or assign meaning to neighboring BulletInf storage. No runtime visual scenario is claimed.

**Production / exact representation.** Normal `src/ecl/EclRun.cpp` extends its existing private `PhotoBulletManagerView` with opaque storage through `+0x27C5B0`, names `anmSpawner` there, and pins the offset with a compile-time assertion. `TH095_ECL_BULLET_ANM_SPAWNER` resolves to that member in normal production. `DIFFBUILD` and `TH095_MATCH_EXACT` expand the accessor back to the historical raw `manager + 0x27C5B0` pointer expression. The three target-high call sites in `EclRunTargetHigh.inl` use only this accessor; no shared header, physical BulletInf layout, calling convention, switch topology, script id, or target-facing relocation is changed.

**Validation.** Focused canonical replay of `src/ecl/EclRun.cpp` passed the configured `ecl-manager-run-ecl` unit 1/1 exact with zero compiler-private label refresh. The first normal-production probe deliberately received no compile credit: EclRun's existing private `PhotoBulletManagerView` exposed only methods, so the new owner accessor initially failed with `anmSpawner is not a member`. The fix stayed inside that private view by adding the offset-asserted owner field; no shared include or ABI was widened. The repeated focused exact replay again passed 1/1 with zero refresh, and a command-local normal probe using pinned VC7.1 13.10.3077 plus the repository `/MT /EHsc /Gs /DNDEBUG /Zi /Gy /GF /Oi /Gr /Od /Ob1 /I src/ecl /I src` profile emitted a 77,946-byte Intel 80386 COFF object. Its temporary object/PDB directory was removed by the same command. `git diff --check` passed. Because the transaction is private to the RunEcl translation unit and does not change a shared owner/header/layout, cold aggregate exact and whole-product gates are deferred to the campaign milestone/final handoff.

**Recovery / analysis state.** This transaction began from committed SEM-175 HEAD `5a044de2d2121111b8f0f45d095ecd14495de2f1` with no staged or tracked-unstaged changes and the same four pre-existing untracked exclusions preserved. `.analysis/` remained at the campaign baseline 3,394,984 bytes; no current-session analysis artifact was created. Fresh target evidence came from the registered read-only attested Ghidra provider.

**Next evidence route.** After checkpoint, rotate away from BulletInf ANM ownership and the recent shared photo-state gates. Prefer a distinct persistent/ABI, replay/input, historical-runtime, sound/resource-lifetime, or non-ANM sibling-protocol family with new TH095-local producer plus independent consumer/cleanup evidence. Keep ANM preload-slot path bytes, ANM VM bit 14, `ReplayScanWorker::unknown010`, replay/input reserved storage, score-header reserved fields including `+0x0C`, Sound/PBG writer-only metadata, TextRenderer RNG prefix, compact-enemy single-ended tail/control storage, shared unresolved bits, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.


### SEM-177 — align PhotoFront and PhotoOverlay gameplay-load gates

**Scope.** Continue the resume audit after SEM-176 by rotating through persistent/ABI, replay/input, worker-lifetime, Supervisor-startup, front-end flag, ANM-VM flag, and PhotoEffect protocol surfaces before returning to one unresolved shared-state coverage gap. Those independent routes produced only bounded Unknown or already-committed semantics. The actionable counterexample is narrower: `PhotoFrontManagerView::OnUpdate`, `PhotoFrontManagerView::OnDraw`, and `DrawPhotoStage` still decoded `g_RuntimeGlobalStateOwner +0xFC` bit 2 through raw shifts in normal production. SEM-055/SEM-111 already establish that physical bit as the PhotoGameTask `gameplayLoadActive` lifetime, while SEM-175 aligned only PhotoCard consumers. This transaction binds the missed PhotoFront/PhotoOverlay callbacks to that established owner meaning. It does not reinterpret bit 2, touch the distinct PhotoStage `+0x25720` bit 2 `firstCaptureFrame`, or assign meaning to any neighboring global-state bit.

**Observed.** Fresh Factory-attested TH095 decompilation of `PhotoFrontManagerView::OnUpdate @ 0x00417970` and `PhotoFrontManagerView::OnDraw @ 0x004179A0` shows both callbacks reading `DAT_004BDEC8 + 0xFC` bit 2 and returning the chain-continue value without calling `Update @ 0x004175B0` or `Draw @ 0x00417920` while the bit is set. Fresh `DrawPhotoStage @ 0x0042C410` first configures gameplay viewport 1, then reads the same `DAT_004BDEC8 + 0xFC` bit 2 and suppresses `PhotoOverlayManagerView::Draw @ 0x0042C220` while it is set. These are three independent callback consumers on the same physical owner.

Fresh `PhotoGameTaskView::Load @ 0x00417D20` provides the producer/lifetime edge: it ORs `4` into the dword at `DAT_004BDEC8 + 0xFC` before waiting for asynchronous capture/ANM work and subsystem initialization, then clears the same bit with `& 0xFFFFFFFB` only on the successful load path. The failure path instead sets bit 3 and leaves the established load-failure protocol to the task update. The three front/overlay callbacks therefore consume the already-proven gameplay-loading lifetime rather than a local rendering flag.

**Corroborated.** SEM-055 originally established shared bit 2 `gameplayLoadActive` from the PhotoGameTask loader and independent subsystem gates. SEM-111 bound the task's own `flags @ +0xFC` representation to the same physical `g_RuntimeGlobalStateOwner` field. SEM-175 later found PhotoCard as another missed consumer, demonstrating that the earlier owner recovery was deliberately not exhaustive. The current repository-history/semantic search found no subsequent record aligning the PhotoFront callbacks or `DrawPhotoStage`. The exact ledger independently describes the two PhotoFront functions as game-task-gated Chain callbacks and `DrawPhotoStage` as a photography-suppression-bit-2 gate.

**Inferred.** PhotoFront update/draw and PhotoOverlay stage drawing participate in the same asynchronous gameplay-load boundary as the already-aligned PhotoCard, Bullet, Item, Stage, and task callbacks. `gameplayLoadActive` is deliberately narrower than a generic rendering pause: it names the producer lifetime, while each consumer retains its own observed effect. In particular, this result says nothing about PhotoStage `firstCaptureFrame`, photo transitions, result-screen state, or capture-active state.

**Unknown / bounded.** Shared global-state bit 8 and all other unresolved bits remain Unknown. This batch does not assign a visual-design reason for why these callbacks are suppressed during loading, does not claim that every consumer of bit 2 has now been enumerated, and does not change the meaning of `PhotoFrontStageStateView::firstCaptureFrame`. `PhotoOverlayManagerView::unknown25714` and all unrelated overlay/front storage remain opaque. No runtime visual scenario is claimed.

**Production / exact representation.** Normal `PhotoFront.cpp` now reads the canonical `PhotoGameTaskView::gameplayLoadActive` member in `OnUpdate` and `OnDraw`; `DIFFBUILD` retains the historical raw shift. Normal `PhotoOverlay.cpp` gives its local shared-global-state projection a layout-preserving bitfield naming only bit 2 `gameplayLoadActive`, leaving bits 0..1 and 3..31 anonymous, and `DrawPhotoStage` consumes that member. Its `DIFFBUILD` branch retains the raw `u32 flags` projection and shift. Both translation units still select their frozen exact implementations at the `TH095_MATCH_EXACT` file boundary, so target-facing source shape and relocations are unchanged.

**Validation.** Focused canonical replay `python3 scripts/replay-exact-units.py --source src/PhotoFront.cpp --source src/PhotoOverlay.cpp` rebuilt all 22 configured units and passed 22/22 exact with zero compiler-private label refresh. Command-local normal-production probes reused each source's canonical manifest profile under pinned VC7.1 13.10.3077 and emitted 32,421-byte `PhotoFront.obj` and 23,596-byte `PhotoOverlay.obj` Intel 80386 COFF objects; the temporary object/PDB directory was removed by the same command. `git diff --check` passed. No shared header, PCH, physical owner, serialized layout, or ABI changed, so cold aggregate exact and whole-product gates are deferred to the next committed campaign milestone/final handoff.

**Recovery / analysis state.** This campaign began from committed handoff HEAD `340822f0d7059b3e0018ccf8325676292867d665` with no staged or tracked-unstaged changes and the same four pre-existing untracked exclusions preserved outside staging. `.analysis/` began at 3,394,984 bytes and no current-session analysis artifact has been created; fresh target evidence came through the registered read-only attested Ghidra provider. Several read-only Factory requests lost transport before returning durable command ids; each received zero evidence credit and live HEAD/worktree state was rechecked before continuing.

**Next evidence route.** After checkpoint, rotate away from shared gameplay-load gates again. Prefer a distinct persistent/ABI boundary, historical-runtime owner, non-ANM resource lifetime, replay/input protocol with a newly discovered field-level consumer, or a sibling interpreter/state family with TH095-local producer plus independent consumer/cleanup evidence. Keep FrontEnd controller bit 4, Supervisor/shared bit 8, `ReplayScanWorker::unknown010`, replay/input reserved bytes, score/replay reserved words, SoundPlayer/PBG writer-only metadata, ANM VM bit 14, ANM preload-slot path bytes, TextRenderer RNG prefix, compact-enemy single-ended tail/control storage, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.


### SEM-178 — bind ANM text visibility publications

**Scope.** Rotate away from SEM-177 shared gameplay-load callbacks into an independent ANM text producer family after additional persistent, weak-API, bullet-owner, and normal-preprocessing audits. The current normal `AnmText.cpp` still ended each of `DrawTextLeft`, `DrawTextRight`, and `DrawTextCentered` with raw `flagsWord |= 1` even though the canonical `AnmVm +0x228` owner already names bit 0 `visible` and TH095-local draw behavior distinguishes that bit from the separate bit-1 `drawEnabled` gate. This transaction aligns only those three text-render publications with the existing visibility owner. It does not reinterpret ANM bit 1, bit 14, script execution, sprite lifetime, or text-buffer ownership.

**Observed.** Fresh Factory-attested TH095 decompilation of `AnmTextManagerView::DrawTextLeft @ 0x00443CE0`, `DrawTextRight @ 0x00443DD0`, and `DrawTextCentered @ 0x00443F80` shows the same terminal operation after formatting and `DrawTextInner`: each ORs `1` into its VM's dword at `+0x228`. Fresh `AnmManager::Draw @ 0x004415A0` independently tests the same physical bit first and returns `-1` while it is clear, before separately testing bit 1 and alpha/render state. Fresh `AnmVm::Initialize @ 0x00401C10` initializes the low flag halfword at `+0x228` to `7`, publishing bits 0, 1, and 2 for a newly initialized VM. Thus the text helpers are genuine post-render producers of the already-separated bit-0 visibility gate.

**Corroborated.** The canonical `AnmVm` layout names bit 0 `visible` and bit 1 `drawEnabled`. ANM-040 / SEM-086 established from `AnmVm::Initialize`, `AnmManager::Draw`, `Draw3D`, and `DrawVertices` that bit 1 is a separate draw-enable gate evaluated only after bit 0 visibility. The exact ANM text ledger already describes `DrawTextLeft` as making the VM visible, while `DrawTextRight` and `DrawTextCentered` end with the same target publication. Repository history shows the raw writes descend from the original ANM-text reconstruction and no later semantic batch aligned them with the canonical bitfield, so this is missed producer representation rather than a new interpretation inferred from naming alone.

**Inferred.** The strongest maintainable statement is that completing one of the three text rendering frontends publishes the receiving VM as `visible`. This is narrower than `drawEnabled`: the text helper does not force bit 1, change render mode, or guarantee that the VM will actually draw if another gate fails. The existing `visible` identifier therefore matches the observed owner protocol without claiming UI policy beyond the target write/read pair.

**Unknown / bounded.** ANM bit 14 remains Unknown: the campaign's fresh full `ExecuteScript @ 0x0043A600` audit observed many `+0x228` flag publications but no bit-14 writer. This batch does not infer whether text rendering historically used a dedicated source method to set visibility, does not rename any text sprite padding, and does not change glyph placement, format-string behavior, texture content, VM lifetime, or script state. No runtime visual scenario is claimed.

**Production / exact representation.** `src/AnmText.cpp` now uses the source-local `TH095_ANM_TEXT_PUBLISH_VISIBLE` abstraction at the three text frontends. Normal production expands it to `reinterpret_cast<AnmVm *>(vm)->visible = 1`, reusing the canonical physical owner already included through `AnmText.hpp`. `TH095_MATCH_EXACT` and `DIFFBUILD` expand it to the historical `vm->flagsWord |= 1`, preserving the target-facing expression and local `AnmTextVmView` layout. No shared header, ABI, object size, calling convention, or relocation owner changes.

**Validation.** Focused canonical replay `python3 scripts/replay-exact-units.py --source src/AnmText.cpp` rebuilt all four configured ANM text units and passed 4/4 exact with zero compiler-private label refresh. A command-local normal-production probe reused the canonical manifest profile under pinned VC7.1 13.10.3077 and emitted a 19,239-byte Intel 80386 COFF `AnmText.obj`; the temporary object/PDB directory was removed by the same command. The existing VC7.1 warnings for the repository's `#pragma var_order` source annotations were unchanged. `git diff --check` passed. No shared header/PCH/layout changed, so cold aggregate exact and whole-product gates are deferred to the campaign milestone/final handoff.

**Recovery / analysis state.** This transaction began from committed SEM-177 HEAD `493769440f2880a1dc65ee3934a31ac0441b15bb` with no staged or tracked-unstaged changes and the same four pre-existing untracked exclusions preserved outside staging. `.analysis/` remains at the campaign-start 3,394,984 bytes; no current-session analysis artifact was created. Fresh target evidence came through the registered read-only attested Ghidra provider.

**Next evidence route.** After checkpoint, rotate away from ANM text visibility and the recent shared gameplay-load family. Prefer a distinct historical-runtime/resource-lifetime owner, persistent or serialized ABI boundary with a new validator/consumer, replay/input protocol with field-level evidence, or non-ANM sibling interpreter/state family. Keep FrontEnd controller bit 4, Supervisor/shared bit 8, `ReplayScanWorker::unknown010`, replay/input reserved bytes, score/replay reserved words, SoundPlayer/PBG writer-only metadata, ANM VM bit 14, ANM preload-slot path bytes, TextRenderer RNG prefix, compact-enemy single-ended tail/control storage, and photo-score bits 5/17/18/19 Unknown absent new TH095-local evidence. The semantic phase remains active-incomplete.


### SEM-179 — type the photography player mode protocol

**Scope.** Rotate away from the recent PhotoCard/BulletInf and ANM-text families and adversarially inspect runtime state protocols whose target behavior was already partly understood but whose maintainable representation remained numeric. The Player object at `g_RuntimePlayerOwner` begins with a four-byte `mode @ +0x0000`. Existing exact PHOTO-012 described the complete four-way switch and SEM-092 named the completion signals produced by modes 2 and 3, but normal `PhotoGame.cpp` and the camera-side Player view still expressed the state domain as raw `0/1/2/3`. This transaction recovers only that four-state mode protocol. It does not change Player ownership, camera modes, replay mode, movement state, completion flags, timer layout, or transition timing.

**Observed.** Fresh Factory-attested TH095 decompilation of `PhotoGameUpdateView::Update @ 0x0042FF60` switches directly on Player `+0x0000`. State 0 moves `playerPosition.y` from 480 toward 400 using the completion timer and, once the timer reaches 60, writes state 1, resets the timer, and falls through. State 1 is the only branch that calls both `UpdateMainState @ 0x0042F190` and `UpdatePhotoCamera @ 0x00430AB0`. State 2 waits for the completion timer to exceed 29, then publishes shared bit 5 and restores game speed to 1.0. State 3 dispatches the established shutdown sequence: enemy reset at timer 4, Bullet/Stage reset at timer 15, and shared bit 6 publication at timer 30. The same function executes the Player ANM VM and advances the completion timer after the state dispatch.

Fresh target `PhotoGameUpdateView::Die @ 0x004306D0` independently writes Player state 2, resets the same completion timer, spawns the death-effect VMs, and sets game speed to 0.5. `PhotoPlayerRuntimeView::CheckBulletCollision @ 0x00430450` independently rejects collisions while Player state is 2 or 3 before otherwise dispatching `Die`; the maintained laser collision and DrawPlayer paths make the same state distinctions. Fresh target `PhotoCameraState::TakePhoto @ 0x00432D10` independently writes Player state 3 and resets the Player completion timer exactly when `photoIndex >= photoLimit`, while disabling the camera state separately. Player construction zeroes the object, so the initial Player mode is state 0 before the state-0 entrance branch runs.

**Corroborated.** PHOTO-012 already establishes the exact four-state `Update` switch and its 60-frame entrance / 30-frame completion structure. SEM-092 / PHOTO-073 independently bind the state-2 and state-3 completion edges to `playerDeathTransitionComplete` and `photoLimitTransitionComplete`; later ResultScreen, ScreenEffect, and FPS-gate consumers confirm those completion bits as distinct shared lifetimes. PHOTO-092 independently establishes the camera `photoIndex/photoLimit` fields whose comparison is the only maintained producer of Player state 3. All of this evidence is TH095-local; no adjacent-game naming is used as authority.

**Inferred.** The narrow normal-production enum is `PhotoPlayerMode`: `PHOTO_PLAYER_MODE_ENTERING = 0`, `PHOTO_PLAYER_MODE_ACTIVE = 1`, `PHOTO_PLAYER_MODE_DEATH_TRANSITION = 2`, and `PHOTO_PLAYER_MODE_PHOTO_LIMIT_TRANSITION = 3`. “Entering” describes the target-observed 60-frame offscreen-to-playfield motion; “active” describes the sole state that runs ordinary movement/gameplay plus camera update; the transition names describe their independently observed producers and completion publications. These are maintainable behavioral names, not claims about original ZUN identifiers.

**Unknown / bounded.** Original enum/type spelling is unknown. This batch does not claim that arbitrary corrupted or external values outside 0..3 have defined behavior, nor that every visual side effect of the death/photo-limit transitions has been exhaustively interpreted. Camera `PhotoCameraMode`, Player movement/camera-tracking submodes, replay mode, shared task bit 8, and the completion-flag business logic outside the established producer/consumer edges are separate domains. No deterministic runtime scenario was executed; the pre-existing untracked runtime scenario/driver files remain excluded recovery state and are not promoted to an Oracle.

**Production / exact representation.** Normal `PhotoPlayerRuntime.hpp` defines `PhotoPlayerMode` and types the shared Player owner `mode @ +0x0000` with it. Normal `PhotoGameUpdateView` and `PhotoCamera.hpp::PhotoGameStateView` use the same enum without changing size or offset. `PhotoGame.cpp` uses state constants for the collision gates, death producer, DrawPlayer gate, and all four `Update` switch cases. `PhotoCamera.cpp` uses the state-3 constant for the photo-limit publication. `TH095_MATCH_EXACT` and `DIFFBUILD` keep four-byte integer fields and the historical numeric `0/1/2/3` expressions through narrowly scoped compatibility macros, so target-facing source shape and decorated ABI remain unchanged.

**Validation.** Fresh target evidence came through the registered read-only Factory-attested Ghidra provider for `0x0042FF60`, `0x004306D0`, `0x00432D10`, and `0x00430450`. Focused canonical replay of `src/PhotoGame.cpp` and `src/PhotoCamera.cpp` passed 33/33 configured units exact with zero compiler-private label refresh. Command-local normal probes using pinned VC7.1 13.10.3077 and the repository `/MT /EHsc /Gs /DNDEBUG /Zi /Gy /GF /Oi /Gr /Od /Ob1 /I src` profile emitted 50,824-byte `PhotoGame.obj` and 58,082-byte `PhotoCamera.obj` Intel 80386 COFF objects; the temporary probe directory was removed in the same command. Because `PhotoPlayerRuntime.hpp` / `PhotoCamera.hpp` are shared headers, the current manifest was immediately cold-replayed in four disjoint 22-source groups containing 226, 201, 217, and 52 units; all 696/696 configured units passed exact with zero private-label refresh. A fresh `scripts/build-whole.py` run compiled all 88 production TUs with pinned VC7.1 and linked/verified a 780,288-byte PE32 i386 GUI image with build-local SHA-256 `871bf14e174188b89dd49b604ea265b1ee64075765db2bc43b92c340e306aed0`. Successful product linkage is not a whole-image-exact or runtime-scenario claim. `git diff --check` passed throughout.

**Recovery / analysis state.** The conversation resumed from committed handoff `f18d7c5e25087d8134146220917e2e33bfb2ab28` with no staged or tracked-unstaged work and the same four pre-existing untracked exclusions preserved. `.analysis/` started at 3,394,984 bytes; no current-session `.analysis/gpt-web/` root or retained analysis artifact was created because semantic target evidence came through bounded provider queries. The first broad baseline command used one obsolete graph-script filename and exited after target/Ghidra/tracking had passed; recovery confirmed unchanged source state, and the repository-native CI path subsequently supplied the real graph validators. Several bounded routes were falsified before this edit—ANM bit 14, ReplayScanWorker `unknown010`, shared task/Supervisor bit 8, FrontEnd bit 4, PBG/Sound single-ended metadata, replay/input reserved fields, compact ECL bits 3/6, and exact-only raw-member aliases—without converting those negatives into completion evidence.

**Next evidence route.** After checkpoint, rotate away from the Player mode protocol. Prefer an independent persistent/serialized field with a newly found validator, a non-ANM resource lifetime, a sound/replay/input protocol only when a real field-level consumer appears, or another sibling interpreter/state domain with a TH095-local producer plus independent consumer. Keep shared task bit 8, Supervisor constructor bit 8, FrontEnd bit 4, ReplayScanWorker `unknown010`, replay/input reserved bytes, score-header `+0x0C`, Sound/PBG writer-only metadata, ANM VM bit 14, ANM preload pathname, TextRenderer RNG prefix, compact single-ended fields, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.


### SEM-180 — bind PhotoGameTask to the replay mode protocol

**Scope.** Rotate from SEM-179 Player-state typing into an independent replay-entry protocol. The live `PhotoGameTaskView` stores a four-byte `replayMode @ +0x120`; the field was already known to survive Supervisor transitions and to feed replay setup, but maintained production still treated it and all task-entry producers as untyped integers. `ReplayManager.hpp` already defines the target-backed three-value `ReplayManagerMode` domain. This transaction binds only the PhotoGameTask-owned field and observed task producers/consumers to that existing protocol while preserving the historical integer factory ABI. It does not change replay file format, replay browser state, ReplayManager object layout, or the semantics of its load-only factory.

**Observed.** Fresh Factory-attested TH095 decompilation of `PhotoGameTaskView::Create @ 0x00417F80` allocates the 0x124-byte task, publishes it at `0x004BDEC8`, and writes the incoming integer argument directly to task `+0x120`. Fresh `PhotoGameTaskView::InitializeSubsystems @ 0x00417A70` later passes that exact dword as the first argument to `ReplayManager::Create @ 0x004345B0` together with the shared replay path. Thus the task field is not a parallel boolean; it is the value forwarded into the ReplayManager mode protocol.

Fresh `SceneSelectControllerView::Update @ 0x00445E80` independently establishes the task producer subdomain. Its normal-game transition calls `PhotoGameTaskView::Create(0)` and later requests Supervisor photo-game state 3. Its replay transition calls `PhotoGameTaskView::Create(1)` and later requests Supervisor replay-start state 7. Fresh `Supervisor::UpdateSceneState @ 0x00425EF0` independently reads task `+0x120`: returning from gameplay to the front end maps zero to front-end entry mode 1 and nonzero to entry mode 2; the result-restart path snapshots the same dword and recreates the task with it; the ordinary retry path explicitly creates the replacement task with 0.

**Corroborated.** The canonical `ReplayManagerMode` domain is `REPLAY_MANAGER_RECORD = 0`, `REPLAY_MANAGER_PLAYBACK = 1`, and `REPLAY_MANAGER_LOAD_ONLY = 2`. ReplayManager exact reconstruction already distinguishes record/playback in `Initialize` and uses mode 2 only in `ReplayManager::Load`, which is a separate no-chain-registration factory. ABI-061 independently records the two FrontEnd calls to the real PhotoGameTask factory with values 0/1. ABI-074 independently establishes `PhotoGameTask +0x120` as the Supervisor transition-owned replay-mode field through target xrefs and historical runnable-product validation. The current target evidence closes the missing data-flow edge from those task producers through `+0x120` into ReplayManager.

**Inferred.** In the PhotoGameTask surface, value 0 is `REPLAY_MANAGER_RECORD` / ordinary new-game recording and value 1 is `REPLAY_MANAGER_PLAYBACK` / replay gameplay. Reusing `ReplayManagerMode` for the task field expresses the observed pass-through protocol rather than inventing a second task-specific enum. The maintained `Create(i32)` parameter deliberately remains integer-typed because the VC7.1 decorated symbol is part of the exact ABI.

**Unknown / bounded.** No TH095 PhotoGameTask producer of `REPLAY_MANAGER_LOAD_ONLY = 2` was observed, so this batch does not claim that mode 2 is a reachable task state. It remains an established ReplayManager-only load path in the audited surface. This batch does not reinterpret FrontEnd entry modes 1/2 as ReplayManager modes; Supervisor merely derives them from whether the task mode is record versus non-record. Original ZUN enum/type names are unknown. No new runtime scenario was executed, and the pre-existing untracked runtime driver/scenario files remain excluded rather than promoted to current validation.

**Production / exact representation.** Normal `PhotoGameTask.hpp` includes the existing `ReplayManagerMode` definition and types `PhotoGameTaskView::replayMode @ +0x120` with it; `TH095_MATCH_EXACT` and `DIFFBUILD` retain `i32`. `PhotoGameTaskView::Create(i32)` keeps its signature and casts only the stored normal field. Normal FrontEnd task creation uses `REPLAY_MANAGER_RECORD` and `REPLAY_MANAGER_PLAYBACK`; historical proxy calls in DIFF/exact keep literal 0/1. Normal Main uses `ReplayManagerMode` for its partial task field and transition locals, names the ordinary retry constant `REPLAY_MANAGER_RECORD`, and tests against record mode when selecting the return-to-front-end entry mode. DIFFBUILD expands the same helper macros to the historical integer expressions. ReplayManager declarations and serialized replay bytes are untouched.

**Validation.** Fresh target evidence came through the registered read-only Factory-attested Ghidra provider for `0x00417F80`, `0x00417A70`, `0x00425EF0`, and `0x00445E80`. Focused canonical replay of Main, FrontEndController, and PhotoGameTask passed 62/62 configured exact units with zero compiler-private label refresh. The first command-local normal Main probe received no compile credit: a preprocessor conditional had been placed inside a macro-call argument list, which VC7.1 rejected before semantic compilation. Recovery confirmed unchanged HEAD, the intended four tracked edits, and no active replay/compiler producer. The source difference was moved into file-level compatibility macros; repeated normal probes then emitted 115,040-byte Main, 37,443-byte FrontEndController, 35,473-byte PhotoGameTask, and 32,541-byte PhotoFront Intel 80386 COFF objects under their canonical pinned-VC7.1 profiles.

Because `PhotoGameTask.hpp` is a shared production header, the current dirty source was immediately cold-replayed in four disjoint 22-source groups containing 226, 201, 217, and 52 units. All 696/696 configured units passed exact with zero private-label refresh. During partition one, the local `origin/main` ref was externally fast-forwarded to the unchanged semantic HEAD `6b81088e4d79f92d592d3eee3793b5f93c3f2f57`; recovery verified HEAD identity, zero divergence, unchanged tracked transaction diff, unchanged exclusion hashes, and no active producer. No push was performed by this campaign. A fresh `scripts/build-whole.py` run then compiled all 88 production TUs with pinned VC7.1 and linked/verified a 780,288-byte PE32 i386 GUI image with build-local SHA-256 `1a9f209a36da38db35b96c61e4f5dc4a29c367cd2a82a12989f4cb0df17abb42`. Product linkage remains separate from whole-image identity and runtime-scenario validation.

**Recovery / analysis state.** This transaction began from clean tracked SEM-179 checkpoint `6b81088e4d79f92d592d3eee3793b5f93c3f2f57`; the four documented pre-existing untracked paths remained excluded. `.analysis/` remained at the campaign baseline 3,394,984 bytes and no current-session analysis artifact was created. The external upstream-ref update changed only the local remote-tracking reference, not repository source or the user-excluded paths.

**Next evidence route.** After checkpoint, rotate away from PhotoGameTask replay-entry typing and the preceding Player mode domain. Prefer a distinct persistent/serialized field with a real validator/reader, a non-ANM resource lifetime, historical runtime ownership, or another sibling protocol with a TH095-local producer plus independent consumer/cleanup edge. Keep shared task bit 8, Supervisor constructor bit 8, FrontEnd bit 4, ReplayScanWorker `unknown010`, replay/input reserved bytes, score-header `+0x0C`, Sound/PBG writer-only metadata, ANM VM bit 14, ANM preload pathname, TextRenderer RNG prefix, compact single-ended storage, and photo-score bits 5/17/18/19 Unknown absent new evidence. The semantic phase remains active-incomplete.


### SEM-181 — type ReplayManager mode storage

**Scope.** Continue from SEM-180 at the downstream owner of the same replay protocol rather than reopening task-entry routing. `ReplayManager.hpp` already defines `ReplayManagerMode` with `REPLAY_MANAGER_RECORD = 0`, `REPLAY_MANAGER_PLAYBACK = 1`, and `REPLAY_MANAGER_LOAD_ONLY = 2`, but the manager's own four-byte `mode @ +0x00` remained an untyped `i32` in maintainable production. This transaction types only that stored owner field. It deliberately preserves the historical integer parameter types of `ReplayManager::Initialize(i32, char *)` and `ReplayManager::Create(i32, char *)`, so no decorated ABI is changed.

**Observed.** Fresh Factory-attested TH095 decompilation of `ReplayManager::Initialize @ 0x004342A0` stores the incoming integer directly at manager `+0x00` before any branch. Value 0 takes the record path: it publishes the singleton, allocates the replay header, input stream, and independent FPS stream, clears those buffers, initializes the replay header/current input state, and seeds the stream cursors. Value 1 takes the playback path: it loads the replay, points the active input cursor into the decompressed stream, restores RNG/player/global-state data from that stream, and uses the loaded FPS stream. Value 2 takes the separate load-only path: it loads the replay and publishes the active input-data pointer without installing the record/playback runtime restoration performed by mode 1.

Fresh `ReplayManager::~ReplayManager @ 0x004344F0` independently consumes the same dword as a lifetime discriminator: the input-data allocation is freed for every mode, but the FPS allocation at `+0x0C` is freed separately only when mode is 0 because record mode owns a standalone FPS allocation. Fresh `ReplayManager::ProcessFrame @ 0x00434830` independently treats mode 0 as the record producer and the nonzero branch as replay-stream consumption. The maintained exact reconstruction supplies two further bounded consumers: `DrawFps` requires `REPLAY_MANAGER_PLAYBACK`, while `ReplayManager::Load` invokes `Initialize(REPLAY_MANAGER_LOAD_ONLY, path)` for the no-chain-registration load factory.

**Corroborated.** SEM-180 independently establishes the upstream task publication into this domain: normal gameplay passes `REPLAY_MANAGER_RECORD` and replay gameplay passes `REPLAY_MANAGER_PLAYBACK`, while no PhotoGameTask producer for load-only mode was observed. The pre-existing `ReplayManagerMode` declaration and the canonical exact ReplayManager family already separate all three constants at the call sites. Current TH095 target behavior therefore supports using that existing enum for the manager-owned storage rather than inventing a second representation. TH08 contributes no naming or acceptance authority to this result.

**Inferred.** `ReplayManager::mode` is the persistent per-instance discriminator for the three observed ReplayManager construction protocols. Typing the normal owner field with `ReplayManagerMode` makes the stored representation agree with its target-observed producer and independent lifetime/stream consumers. This is a maintainable reconstruction choice; it is not a claim that the original TH095 source declared the field with this exact enum type or spelling.

**Unknown / bounded.** Original ZUN enum/type names are unknown. `Initialize` establishes behavior only for observed values 0, 1, and 2; this batch does not define invalid-value behavior or reinterpret the broad nonzero branch inside `ProcessFrame` as proof that every nonzero integer is a valid playback mode. It does not change replay serialization, the mode-dependent `fpsData` ownership established by SEM-171, task reachability of load-only mode, or any replay-browser state. No new runtime replay scenario was executed; runtime-scenario validation remains a separate unvalidated plane.

**Production / exact representation.** Normal `ReplayManager::mode @ +0x00` is now `ReplayManagerMode`. `TH095_MATCH_EXACT` and `DIFFBUILD` retain `i32` storage. Normal `Initialize(i32 mode, char *path)` stores `static_cast<ReplayManagerMode>(mode)`; DIFFBUILD retains the historical direct integer assignment. `Initialize` and `Create` keep integer parameters, so their VC7.1 decorated names and public ABI are unchanged. The field remains four bytes at offset zero and all following owner offsets are unchanged.

**Validation.** Focused canonical replay of `src/ReplayManager.cpp` rebuilt all twelve configured ReplayManager units and passed 12/12 exact with zero compiler-private label refresh. A command-local normal-production probe using pinned VC7.1 13.10.3077 and the canonical `/MT /EHsc /Gs /DNDEBUG /Zi /Gy /GF /Oi /Gr /Od /Ob1 /I src` profile emitted a 36,304-byte Intel 80386 COFF object; the temporary object/PDB was removed by the same command. Because `ReplayManager.hpp` is shared, the dirty current source was then cold-replayed across four disjoint manifest-derived partitions of 174 units each. All 696/696 configured units passed exact with zero private-label refresh. A fresh `scripts/build-whole.py` run compiled all 88 production TUs with pinned VC7.1 and linked/verified a 780,288-byte PE32 i386 GUI image with build-local SHA-256 `a43c2df9043a9a43f76e1f61136c168a70397ee57a5b59a7664765c371c8401b`; successful linkage is not a whole-image exact claim. The first monolithic aggregate replay request lost the Factory transport without a durable command id and receives no validation credit. Mandatory recovery confirmed the same HEAD/two-file transaction and no surviving replay/compiler producer before the four durable partitions were run.

**Recovery / analysis state.** The campaign began from committed handoff HEAD `dee282c33dbb75ca0545d3115cd421a6e4f6a7bd` with no staged or tracked-unstaged changes and the same four documented pre-existing untracked exclusions. `.analysis/` began and remains at 3,394,984 bytes; no current-session `.analysis/gpt-web` root or retained analysis artifact was created. The initial handoff-directed `ResultScoreEntryView` audit found no new field-level reader or validator for the remaining anonymous score-entry holes: the parser and writer operate on those bytes through whole-record copy/checksum, while maintained independent field accesses are already covered by committed score semantics. That bounded negative result was treated as a routing plateau, not phase completion.

**Next evidence route.** Rotate away from ReplayManager mode typing and the already-audited score-entry holes. Prefer a different state/resource/persistent owner where current TH095 evidence supplies a producer plus independent consumer or cleanup edge. Do not reopen known exact-facing aliases merely because lexical debt reports them: screen-shake `unknown020/024`, PhotoCamera `unknownbb8`, ScoreFileHeader `unknown008`, and several shared-photo `unknownFlag*` declarations already have maintainable normal representations. Keep score-header `+0x0C`, replay/input reserved bytes, ReplayScanWorker `unknown010`, ANM VM bit 14, compact-enemy single-ended control bits, Sound/PBG writer-only metadata, and other single-ended storage Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.


### SEM-182 — type PhotoCamera mode storage

**Scope.** Rotate from SEM-181 replay ownership into the independent photography-camera state machine. The maintained header already defines `PhotoCameraMode` values 0..4 and the exact `UpdatePhotoCamera` body uses those constants throughout, but `PhotoCameraState::mode @ +0x000` itself remained a bare `i32` in normal production. This transaction types only that owner field. It does not reinterpret the camera's charge UI substate, focus/capture flags, Player mode, replay mode, or any timer/VM storage.

**Observed.** Fresh Factory-attested TH095 decompilation of `PhotoCameraState::BeginCapture @ 0x00432730` reads the first dword of the camera object, returns without changing state unless it is zero, and on the accepted path writes 1 before resetting `modeTimer`, creating the capture VMs, clearing stale charge VMs, and optionally starting camera-focus audio. This establishes the `TRACKING=0 -> CHARGING=1` producer edge.

Fresh `PhotoCameraState::TakePhoto @ 0x00432D10` consumes the active capture state after removing the capture VMs and performing the photo-score/capture work. If `photoIndex < photoLimit`, it writes camera state 2. Otherwise it clears charge, writes camera state 4, independently writes Player state 3, and resets the Player completion timer. Fresh `PhotoCameraState::CancelCapture @ 0x00433000` likewise removes the capture VMs, restores charge to 0.5, clears alternate-capture state, and writes camera state 2. Thus ordinary shot completion and cancellation converge on the same camera state while the terminal photo-limit path remains distinct.

Fresh `UpdatePhotoCamera @ 0x00430AB0` independently dispatches exactly five switch values from camera `+0x000`. Case 0 performs tracking and charge/capture-start logic; case 1 updates the viewfinder and either completes or cancels capture; case 2 runs the captured-result phase and, once `modeTimer` passes the 60-frame boundary, writes state 3 and resets the timer; case 3 returns to state 0 after the 20-frame recovery boundary while continuing charge recovery; case 4 forces charge to zero and retires/interrupts the active viewfinder VMs. These target-local transitions match the existing `PHOTO_CAMERA_TRACKING/CHARGING/CAPTURED/RECOVERING/DISABLED` constants without requiring an inferred sixth state.

**Corroborated.** PHOTO-003 already records the exact BeginCapture tracking-to-charging transition, and PHOTO-004 establishes the complete canonical exact five-state `UpdatePhotoCamera` body. SEM-179 explicitly kept camera mode separate while typing the four-state Player coordinator, so this batch closes a distinct owner representation rather than extending Player mode semantics. Existing normal PhotoGame logic independently tests `camera.mode != PHOTO_CAMERA_CHARGING` before Player-side camera handling. TH08 is not used to name or accept this state domain.

**Inferred.** The narrow maintainable interpretation of camera `+0x000` is the per-camera state-machine discriminator represented by the already-established `PhotoCameraMode` enum. Typing the normal storage with that enum makes the owner layout agree with its producers and independent switch/caller consumers. This is a reconstruction type choice; it does not claim that the original TH095 source used the same enum name or declaration.

**Unknown / bounded.** Original ZUN enum/type spelling is unknown, and behavior for corrupted values outside 0..4 is not defined by this batch. State 2 is named `CAPTURED` according to the existing reconstruction even though cancellation also enters it; the accepted semantic claim is the observed shared post-capture/cancel state and its timed transition, not an assertion about the original developer's wording. This batch does not reinterpret charge/focus flags, the charge UI two-bit protocol, `photoIndex/photosTaken/photoLimit`, or Player photo-limit completion. No deterministic runtime capture scenario was executed; runtime-scenario validation remains separate and unvalidated.

**Production / exact representation.** Normal `PhotoCameraState::mode @ +0x000` is now `PhotoCameraMode`. `TH095_MATCH_EXACT` and `DIFFBUILD` retain the historical `i32` field. The enum and integer are both four bytes under the pinned compiler, so `cameraOffset @ +0x004` and every following camera member retain their established offsets. No function signature, decorated symbol, persistent format, or calling convention changes.

**Validation.** Focused canonical replay of the three direct camera-header consumers passed all 39 configured units exact with zero compiler-private label refresh: PhotoCamera 11/11, PhotoGame 22/22, and PhotoStage 6/6. Command-local normal probes under pinned VC7.1 emitted 58,200-byte PhotoCamera, 50,844-byte PhotoGame, and 48,323-byte PhotoStage Intel 80386 COFF objects; each temporary object/PDB was removed by the same command. Because `PhotoCamera.hpp` is shared, the dirty current source was cold-replayed across the same four disjoint 174-unit manifest partitions; all 696/696 configured units passed exact with zero private-label refresh. The first partition-three submission lost Factory transport before returning a durable command id and receives no validation credit; mandatory recovery confirmed HEAD `77addf1edf6d7933952c72f940c821523fa55f69`, the intended one-file diff, and no surviving replay/compiler producer before the partition was rerun successfully. A fresh `scripts/build-whole.py` run then compiled all 88 production TUs with pinned VC7.1 and linked/verified a 780,288-byte PE32 i386 GUI image with build-local SHA-256 `0086226f6be821ac60b5e024a63140e0bd56dbc2fc7adacf4a3bf3464d7fbc18`; successful linkage is not a whole-image exact claim.

**Recovery / analysis state.** This transaction began from committed SEM-181 checkpoint `77addf1edf6d7933952c72f940c821523fa55f69` with no staged or tracked-unstaged changes and the same four documented pre-existing untracked exclusions. No current-session `.analysis` artifact was created; all fresh semantic observations came from the registered read-only target-attested Ghidra provider, while exactness came from the canonical replay lane. The earlier score-entry, FrontEnd raw-offset, Background padding, ReplayBrowser `+0x04`, shared-photo declaration-only aliases, and exact-facing PhotoCamera `unknownbb8` routes remain bounded negative routing results rather than newly named semantics.

**Next evidence route.** Rotate away from camera and Player mode/state typing. Prefer a separate finite state/resource/persistent protocol whose owner is still weakly represented but has a TH095-local producer plus an independent consumer/cleanup edge. A useful adversarial starting point is a non-camera submode such as Player movement/camera-tracking control only if its numeric domain has multiple independent target producers and consumers; otherwise rotate to a resource or persistent ABI owner. Do not reopen known single-ended storage or exact-facing compatibility aliases. The semantic phase remains active-incomplete.


### SEM-183 — type Player movement direction storage

**Scope.** Follow SEM-182's adversarial next route into the Player-side movement submode rather than reopening the camera mode state machine. The live Player owner at `g_RuntimePlayerOwner` stores a four-byte value at `+0x02D4`. `PhotoGameUpdateView::UpdateMainState` already used a source-local nine-value `PhotoPlayerMovementDirection` enum when assigning and switching on that field, while the canonical `PhotoPlayerRuntimeView` and the camera-side `PhotoGameStateView` still exposed the same storage as anonymous padding or bare `i32`. This transaction makes the movement-direction domain a shared normal-production owner representation. It does not reinterpret `cameraTrackingMode @ +0x02D8`, Player mode, PhotoCamera mode, input-history storage, movement speeds, or camera tracking policy.

**Observed.** Fresh target-attested Ghidra decompilation of `PhotoGameUpdateView::UpdateMainState @ 0x0042F190` reads Player `+0x02D4`, then writes only values 0 through 8 from the directional input-mask decision tree. The observed mapping is value 0 for no direction, 1 up, 2 down, 3 left, 4 right, 5 up-left, 6 up-right, 7 down-left, and 8 down-right. The same target function independently consumes that dword in two eight-case switches: the focused branch maps values 1..8 onto focused axis/diagonal speed signs, while the normal branch maps the same values onto normal axis/diagonal speed signs. Camera charging forces the field to zero before those movement switches.

Fresh target `UpdatePhotoCamera @ 0x00430AB0` supplies an independent consumer on the same published Player owner `0x004C4E70`: the tracking path tests `Player+0x02D4` for nonzero when selecting viewfinder interpolation behavior, and the captured-result path separately tests the same field against values 0, 1, and 2. Those reads are physically distinct from the neighboring `Player+0x02D8` camera-tracking control value, which the same target function also consumes. The owner and domain are therefore not inferred from source naming alone.

**Corroborated.** The canonical exact `UpdateMainState` unit is already byte/relocation exact and the maintained source-local constants match every target-observed producer value and both target switch tables. SEM-179 established the enclosing `g_RuntimePlayerOwner` representation and deliberately left movement submodes separate; SEM-182 likewise kept this Player-side field outside the camera-mode transaction. Existing Player-position, collision, camera, and photo-counter consumers independently establish `0x004C4E70` as the same Player object. All accepted field semantics in this batch are TH095-local; TH08 contributes no naming or acceptance authority.

**Inferred.** The narrow maintainable type is `PhotoPlayerMovementDirection` with the nine behavioral values already used by `UpdateMainState`. Promoting that enum into the normal shared Player runtime view expresses one target-observed producer/consumer protocol instead of leaving three projections of the same dword with different strength. The names describe movement direction only; they do not imply that the original TH095 source used this enum spelling or that the field is a general input-state owner.

**Unknown / bounded.** Original ZUN enum/type names are unknown, and behavior for corrupted values outside 0..8 is not defined by this batch. `UpdatePhotoCamera`'s tests of movement values 0/1/2 do not establish a new camera semantic for those values beyond their already-proven direction identity. `cameraTrackingMode @ +0x02D8` remains a separate finite-state candidate: current target evidence shows writes of 0/1 and a promoted value 2 plus independent 0/1/2 consumers, but this transaction deliberately does not name that protocol. No deterministic gameplay/runtime scenario was executed; runtime-scenario validation remains a separate unvalidated plane.

**Production / exact representation.** Normal `PhotoPlayerRuntime.hpp` now owns the `PhotoPlayerMovementDirection` declaration and exposes `PhotoPlayerRuntimeView::movementState @ +0x02D4` with a compile-time offset assertion while preserving the surrounding Player layout. Normal `PhotoGameUpdateView::movementState` and `PhotoCamera.hpp::PhotoGameStateView::movementState` use the same enum. `TH095_MATCH_EXACT` and the compatibility `DIFFBUILD` surfaces retain four-byte integer storage; `PhotoGame.cpp` keeps the historical local enum declaration only for the `DIFFBUILD` compatibility branch so its numeric source vocabulary remains available without widening shared exact-facing headers. No function signature, decorated symbol, owner address, object size, calling convention, persistent format, or movement algorithm changes.

**Validation.** Fresh target evidence was generated through the repository's target-attesting Ghidra wrapper for `0x0042F190` and `0x00430AB0`. The first focused exact replay exposed compiler-private label drift in `PhotoCamera.cpp` because a new offset assertion remained visible to the exact header surface. No manifest/private-label refresh was accepted. The assertion was restricted to normal production, after which focused replay passed PhotoCamera 11/11 and PhotoGame 22/22, 33/33 total, with zero private-label refresh. A first monolithic aggregate replay later lost Factory transport without a durable command id and receives no exact credit; mandatory recovery found unchanged source, unchanged excluded files, and no surviving producer. Four durable disjoint 22-source partitions then passed 226 + 201 + 217 + 52 = 696/696 configured exact units with zero refresh. After a final source-compatibility guard was added in `PhotoGame.cpp`, the current source was replayed again for all 22 PhotoGame units and remained 22/22 exact with zero refresh; the shared headers had not changed since the 696-unit aggregate.

The shared-header/product risk was also closed in the reconstructed Windows i386 lane. The final current-source `scripts/build-whole.py` run cold-compiled all 88 production translation units with pinned VC7.1 and linked/verified a 780,288-byte PE32 GUI image with build-local SHA-256 `3f1eda74e58839c9156da0dd39e68345314d5aae332c12e053e029e4e8331582`. Successful linkage is production compile/link closure, not target whole-image identity or runtime-scenario validation. Target-independent CI passed all 43 tests, tracking remained 697 source-present / 696 exact, the match-unit graph remained 696 units, and `git diff --check` passed. A standalone global `/DDIFFBUILD` compile experiment was also attempted but is not a repository-supported compile profile: it failed on pre-existing runtime-global/input-history compatibility dependencies unrelated to this field, received no validation credit, and its command-local temporary directory was removed. The repository's supported exact lane defines compatibility state through `TH095_MATCH_EXACT`/late `DIFFBUILD`, which is covered by the exact replays above.

**Recovery / analysis state.** This campaign began from committed handoff HEAD `930bc048c9f55674a8797312fcab7da6f0a10400` with no staged or tracked-unstaged changes. The same four pre-existing untracked paths were classified and preserved outside staging throughout: user-owned unrelated `droid.resume.txt`, unrelated runtime experiments `config/runtime-scenarios.json` and `scripts/runtime-diff.py`, and unknown-origin generated-looking `EnemyManagerUpdate.i`. Their hashes stayed unchanged across recovery gates. `.analysis/` began at 3,394,984 bytes. Fresh target decompiles were bounded under manifested current-session root `.analysis/gpt-web/20260914-th095-movement-direction/`, temporarily raising `.analysis/` to 3,424,066 bytes with only 8,867-byte `update-main-state.c`, 19,229-byte `update-photo-camera.c`, and its small manifest. After the target conclusions were reduced into this tracked record and no active producer/reference remained, only those current-session files were removed explicitly; `.analysis/` returned to exactly 3,394,984 bytes. No retained current-session large artifact exists and legacy/shared analysis roots were untouched.

**Next evidence route.** After checkpoint, keep the semantic phase active-incomplete and rotate one field to the adjacent but distinct Player `cameraTrackingMode @ +0x02D8` only if fresh TH095 evidence can distinguish the full 0/1/2 behavioral domain without borrowing names from movement or camera mode. The existing target lead is bounded: `UpdateMainState` publishes 0/1 from input and promotes the focused branch to 2 under the secondary input, while `UpdatePhotoCamera` independently distinguishes 0, 1, and 2 in tracking-radius/viewfinder behavior. If that evidence cannot support stable behavioral names, leave the field Unknown and rotate to a separate resource/persistent/ABI owner. Keep shared task bit 8, Supervisor constructor bit 8, FrontEnd bit 4, `ReplayScanWorker::unknown010`, replay/input reserved bytes, score-header `+0x0C`, Sound/PBG writer-only metadata, ANM VM bit 14, ANM preload pathname, TextRenderer RNG prefix, compact single-ended storage, and photo-score bits 5/17/18/19 Unknown absent new distinguishing evidence. The semantic phase remains active-incomplete.
