...
| TH095 EnemyInf and compact enemy | `PhotoEnemyManager.hpp`, `EnemyManagerUpdate.cpp`, `EnemyManagerTask.cpp`, `PhotoRuntime.cpp`; TH08 compatibility ABI in `EnemyManager.hpp` | `PhotoEnemyManager.hpp` is the single profile-independent `0x26AE30` normal owner. Primary `enemyAnm @ +0x4DF8` is proved; `+0x4DFC` remains opaque. The inline `0x4CC0` compact element is lifetime-owned through fixed-size slots but its duplicate observation views are the next lane. Do not treat the 481-slot `EnemyManager.hpp` layout as TH095 EnemyInf. |
