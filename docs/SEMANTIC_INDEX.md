...
| ECL interpreter | `ecl/EclRun.cpp`, `ecl/EclRunLow.inl`, `ecl/EclRunHigh.inl`, `EclManager.hpp`, `EclExtended.cpp` | Normal EclExtended uses canonical ANM handles/loaded owners, `Float3`, and `PhotoEffectManagerView`; old receiver spellings are isolated in named emission inlines. Primary opcode dispatch and remaining photo-owner projections still contain substantial debt; do not infer TH08 opcode names. |
...
| Effects and bullets | `EffectManager.*`, `ScreenEffect.*`, `BulletManager.*`, `PhotoEffectRuntime.hpp` | EclExtended's manager call now uses the canonical PhotoEffect owner and `nextId`; compact BulletInf layout ownership is the next consolidation candidate. Keep variant/raw instruction representations distinct. |
