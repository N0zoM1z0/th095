...
| MATCH-007 | exact | Seven ResultScreen/replay-UI units reproduce 1,433 authored bytes and all 20 relocations. They prove a 12-byte timer at `ResultScreen+0x08`, twenty-five `0x2CC`-byte VM views at `+0x18`, the replay cursor at `+0x4604`, and twenty replay pointers at `+0x6CE8`. | Pinned VC7.1 relocation replay and compile-time layout assertions |
