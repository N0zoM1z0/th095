...
| `0x004149F0..0x004162F0` | 591 exact authored bytes | manager factory / four draw groups / calc and draw gates | five exact EnemyManager task-shell functions | Canonical `PhotoEnemyManager.hpp` 0x26AE30 owner at global `0x004BDDC0`, Chain priorities 12/10, draw-group heads `+0x4DC0`, primary enemy ANM `+0x4DF8`, opaque `+0x4DFC`, 128 compact slots `+0x4E00` |
...
| `0x004020C0..0x00402E90` | 2,515 exact authored bytes | Background construction/destruction, lifecycle callbacks, loader, VM updater | eleven exact Background functions | `Background.hpp` is the single profile-independent 0x201C owner; exact ctor+dtor prove timer arrays, eleven inline ANM VMs, three ownership frees and vector member destruction |
...
| `0x00404C80..0x0040860D` | canonical `0x27C5B8` owner plus exact authored behavior | lifecycle, update/draw, spawn, capture-list builder, photographed-bullet clear, nearby scoring | profile-independent `PhotoBulletManager.hpp`; exact BulletManager surface | TH095 BulletInf lane: one normal owner with `0x641` inline bullets at `+0x4C`, Chain nodes, slot-six `bullet.anm`, capture AABB list, bullet clear/effect conversion, and proximity score |
