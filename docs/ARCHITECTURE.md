...
| `0x004537F0..0x00454E42` | 3,811 exact authored bytes | ZUN-modified DirectSound streaming/wave layer | eighteen exact `CSound`/`CStreamingSound`/`CWaveFile` functions | Notification buffers, nonlinear volume and four fades, refill/loop hub, `ThBgmFormat` raw file/memory I/O; Microsoft scaffold and compiler destructor classified separately |
