...
supplies 1,830 provisional function candidates. Eighty-one canonical units now
reproduce 73,481 authored bytes exactly, including the complete 17,018-byte
...
target-local audit has now admitted 57,596 additional bytes
from the photography/camera, replay/menu, and gameplay/resource clusters, so
exact coverage is 58.40% of the expanded 125,815-byte authored set. The global
