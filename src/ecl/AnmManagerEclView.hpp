...
#if 0 // TH08 layout retained only as private reference; TH095 is defined below.
struct AnmVmBase
{
...
struct AnmVm
{
    ...
    ZunTimer currentTimeInScript;
    ZunTimer waitTimer;
    ZunTimer interpCurrentTimers[7];
    ZunTimer interpEndTimers[7];
    u8 interpModes[7];
    u8 unknown117;
    i32 intVar0;
    i32 intVar1;
    ...
    ZunTimer interruptReturnTime;
    AnmRawInstr *interruptReturnInstruction;
    Float3 positionInitial;
    Float3 positionFinal;
    Float3 rotationInitial;
    Float3 rotationFinal;
    Float2 scaleInitial;
    Float2 scaleFinal;
    ZunColor color1Initial;
    ZunColor color1Final;
    ZunColor color2Initial;
    ZunColor color2Final;
    void (__fastcall *positionCallback)(AnmVm *);
    u32 unknown2ac;
    Float3 alternatePosition;
    i32 timeOfLastSpriteSet;
    u8 unknown2c0[0x0c];

    void SetInterrupt(i16 interrupt) { this->pendingInterrupt = interrupt; }
    void SetZRotation(f32 z)
    {
        this->rotation.z = z;
        this->updateRotation = 1;
...
C_ASSERT(sizeof(AnmVm) == 0x2cc);
C_ASSERT(offsetof(AnmVm, anmFile) == 0x230);
C_ASSERT(offsetof(AnmVm, scriptIndex) == 0x23a);
C_ASSERT(offsetof(AnmVm, pendingInterrupt) == 0x22e);
